    ...
    public static void ProcessWcrDirectory()
    {
        ...
        try
        {
            ...
            if (System.IO.Directory.Exists(sPath + @"\"))
            {
                ...
                else if (!RRDM4ATMs.PublicFn.DeleteFiles(sTargetDirectory))
                {
                    ...
                    if (oMatch.Success)
                    {
                        ...
                        if (resultFile.Extension == sOldExtension)
                        {
                            ...
                            if ((Int64.Parse(dtCreationDate.ToString("yyyyMMdd")) >= Int64.Parse(dtFromDate.ToString("yyyyMMdd"))) && (Int64.Parse(dtCreationDate.ToString("yyyyMMdd")) <= Int64.Parse(dtToDate.ToString("yyyyMMdd"))))
                            {
                                ...
                                sDestination = sTargetDirectory + @"\" + RRDM4ATMs.PublicFn.fnRight(("00000000" + sAtmLevel), 8) + "_" + sYearLevel + RRDM4ATMs.PublicFn.fnRight(("00" + sMonthLevel), 2) + sDayLevel + "_EJ_WCR.00" + resultFile.Name.Substring(14, 1);
