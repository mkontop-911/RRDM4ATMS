...
{
    public partial class Form20_NEW : Form
    {
        ...
        public Form20_NEW(string InOperator,string InSignedId, int SignRecordNo,  
                               string InChosenUserId, int InFunctionNo, int InApplication, bool IsForCIT)
        {
            ...
            if (WSignedId == "ADMIN-BDC" & W_IsForCIT == false)
            {
                panelAdmin.Show(); 
            }
            else
            ...
            if (Us.User_Is_Checker)
            {
                IsChecker = true;
                MessageBox.Show("Checker not allowed to Open users");
                return; 
            }
                ...
                if (WFunctionNo == 1) // New ... so create password
            {
                ...
                checkBoxMakeUserInactive.Enabled = false;
                checkBoxMakeUserActive.Enabled = false;
            }

            if (WFunctionNo == 2) // UPDATE USER
            {
                checkBox4.Checked = false;
                checkBox3.Checked = false;
                checkBox5.Checked = false;
                ...
                if (Us.Is_Approved)
                {
                    checkBoxMakeUserInactive.Enabled = true; // Maybe wish to make it inactive. 
                    checkBoxMakeUserActive.Enabled = true;
                }
                else
                {
                    checkBoxMakeUserInactive.Enabled = false;
                ...
                {
                    Saved_NoOfNotes = Cn.TotalNotes;
                    New_NoOfNotes = Saved_NoOfNotes; // keep equal and see if later is creater 
                    labelNumberNotes3.Text = Saved_NoOfNotes.ToString();
                }
            ...
            {
                Us.ReadUsersRecord_FULL(WChosenUserId); // Read USER Chosen
                ...
                textBoxMobile.Text = Us.MobileNo; // String
                ...
                checkBoxMakeUserInactive.Checked = Us.UserInactive;
                ...
                if (WSignedId == "ADMIN-BDC")
                {
                    ...
                    {
                        radioButtonMake_Maker.Checked = true;
                        radioButtonMake_Checker.Checked = false;
                    }
                    if (Us.User_Is_Checker == true)
                    {
                        radioButtonMake_Maker.Checked = false;
                        radioButtonMake_Checker.Checked = true;
                    }
                }
            ...
            if (WFunctionNo == 1) // Show Information For New 
            {
                ...
                if (WSignedId == "ADMIN-BDC")
                {
                    checkBoxMakeUserInactive.Checked = false; // NEW USER IS INACTIVE TILL AUTHORISED 
                    checkBoxMakeUserInactive.Enabled = false;

                }
                else
                {
                    checkBoxMakeUserInactive.Checked = true; // NEW USER IS INACTIVE TILL AUTHORISED 
                    checkBoxMakeUserInactive.Enabled = false;
        ...
        {
            if (WSignedId == "ADMIN-BDC" & W_IsForCIT == true)
            {
                // Check that only CIT is selected 
                if (comboBox3.Text != "CIT Company")
                {
                    MessageBox.Show("You can only select type CIT Company");
                    return; 
                }
            }
            if (comboBox3.Text == "")
            ...
            // Check if info was Collected
            if (WSignedId == "ADMIN-BDC")
            {
               if (radioButtonMake_Maker.Checked ==false & radioButtonMake_Checker.Checked == false)
                {
                    MessageBox.Show("Please select Maker OR Checker");
                    return;
                }
            }
            // Read Info from active directory 
            ...
            Us.MobileNo = textBoxMobile.Text;
            ...
            if (checkBoxMakeUserInactive.Checked == true)
            {
            ...
            {
                string whattype = "";
                if (radioButtonMake_Maker.Checked == true)
                {
                    Us.User_Is_Maker = true;
                    whattype = "As MAKER"; 
                }

                if (radioButtonMake_Checker.Checked == true)
                {
                ...
                Us.UpdateTail_For_Admin_Work((textBoxUserId.Text).Trim());

                // Insert Record In History 

                Us.Insert_In_Maintenance_History((textBoxUserId.Text).Trim());

                string WMessage = "USER.."+ (textBoxUserId.Text).Trim() + "..Added.." + whattype;
                //AUDIT TRAIL 
                //AUDIT TRAIL 
                string AuditCategory = "Audit_Trail_For_User_Mgmt";
                string AuditSubCategory = (textBoxUserId.Text).Trim(); // Name of user
            ...
            if (IsMaker==true)
            {
                ...
                string WMessage = "USER.." + (textBoxUserId.Text).Trim() + "..User ADDED.. ";
                //AUDIT TRAIL 
                //AUDIT TRAIL 
                string AuditCategory = "Audit_Trail_For_User_Mgmt";
                string AuditSubCategory = (textBoxUserId.Text).Trim(); // Name of user
        ...
        // UPDATE User
        bool HadToMakeInactive;
        bool HadToUndoInactive;
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            // Check that note of reason had inserted 
            if ((Us.UserInactive==false & checkBoxMakeUserInactive.Checked == true)
                || (Us.UserInactive == true & checkBoxMakeUserInactive.Checked == false)
                )
            {
                // It was approved as Active and we want to make him Inactive
                if (New_NoOfNotes > Saved_NoOfNotes)
                {
                    // OK a Note of reason has been added
                    if (Us.UserInactive == false & checkBoxMakeUserInactive.Checked == true)
                    {
                        HadToMakeInactive = true;
                    }
                    else
                    {
                        HadToMakeInactive = false;
                    }

                    if (Us.UserInactive == true & checkBoxMakeUserInactive.Checked == false)
                    {
                        HadToUndoInactive = true; 
                    }
                    else
                    {
                        HadToUndoInactive = false;
                    }
                }
                else
                {
                    MessageBox.Show("Please enter reason of Updating in Notes");
                    return;
                }
            }
            ...
            string Telephone = textBoxMobile.Text;

            if (Telephone.Length == 11)
            {
                // OK 
                if (Telephone.Substring(0,2) != "01")
                {
                    MessageBox.Show("Mobile Number should start with 01");
                    return; 
                }
            }
            else
            {
                MessageBox.Show("Mobile Number should be 11 digits ");
                return;
            }
            ...
            // Read User - If 1 it doesnt exist if 2 exist 
            Us.ReadUsersRecord_FULL(Us.UserId);
            ...
            {
                Us.ReadUsersRecord_FULL(WSignedId);// Get email of the creator = controller
            ...
            if (WSignedId == "ADMIN-BDC")
            {
                ...
                string WMessage = "ADD OR UPDATE USER Checker Or Maker .. " + (textBoxUserId.Text).Trim();
                //AUDIT TRAIL 
                //AUDIT TRAIL 
                string AuditCategory = "Audit_Trail_For_User_Mgmt";
                string AuditSubCategory = "ADD OR UPDATE USER By ADMIN";
                string AuditAction = "ADD OR UPDATE-USER.. " + (textBoxUserId.Text).Trim();
                string Message = WMessage;
                GetMainBodyImageAndStoreIt(AuditCategory, AuditSubCategory, AuditAction, WSignedId, Message);
            }
            ...
            if (IsMaker)
            {
                ...
                if (checkBoxMakeUserInactive.Checked == true) // Leave it here
                {
                    Us.UserInactive = true;
                }
                else
                ...
                Us.HadToMakeInactive = HadToMakeInactive;
                Us.HadToUndoInactive = HadToUndoInactive; 
        ...
        private void comboBox4_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            ...
            Us.ReadUsersRecord_FULL(TempValue);
...
// Branch 
        private void textBoxBrachId_TextChanged(object sender, EventArgs e)
        {
            if (textBoxBrachId.Text != "")
            {
                ...
                {
                    if (textBoxBrachId.Text == "001")
                    {
                        Bb.ReadBranchByBranchId("0" + textBoxBrachId.Text);
                        if (Bb.RecordFound)
                        {
                            textBoxBrachId.Text = "0" + textBoxBrachId.Text;
                            textBoxBranchName.Text = Bb.BranchName;
                        }
                        else
                        {
