...
{
    public partial class Form502_Load_And_Match_BDC_MULTI : Form
    {
        ...
        RRDMMatchingOfTxns_V02_MinMaxDt_BDC_4_MULTI_CALLER Mtc = new RRDMMatchingOfTxns_V02_MinMaxDt_BDC_4_MULTI_CALLER();
        ...
        public Form502_Load_And_Match_BDC_MULTI(string InSignedId, int SignRecordNo, string InOperator
        ...
        private void buttonDoMatching_Click(object sender, EventArgs e)
        {
                          ...
                          == DialogResult.Yes)
            {
                ...
                //    {
                //        DeleteDate = HST_DATE.AddDays(1);
                //        MessageBox.Show("Transactions will be deleted prior to HST Date.." + DeleteDate.ToShortDateString());
                //    }
                //    else
                //    {
                //        DeleteDate = WFirstCut_Off_Date; // Trans before this date will be deleted
                ...
                //{
                //    DeleteDate = WFirstCut_Off_Date; // Trans before this date will be deleted
                //    MessageBox.Show("Transactions will be deleted prior to First Cycle Date.." + DeleteDate.ToShortDateString());
                //}
            ...
            //
            Mtc.MatchReadyCategoriesUpdateForAtms_Thread_MULTI_Caller(WOperator, WSignedId,
                                           WReconcCycleNo);

            //MessageBox.Show("MultiThreading Has finished");

            // THREADS HAVE FINISH
            // 
            RRDM_THREADS Tr = new RRDM_THREADS();

            string WCriteria = " WHERE StatusFromRRDM <> 1 OR Status <> 1  "; 

            Tr.ReadThreadsAndFillTable(WCriteria); 

            if (Tr.ThreadsTable.Rows.Count > 0)
            {
                Tr.PopulateThreadErrorTable(Tr.ThreadsTable);

                MessageBox.Show("There are errors in multithreading ATMS " + Environment.NewLine
                                + "Number of Threads In Error are:.."+ Tr.ThreadsTable.Rows.Count.ToString() + Environment.NewLine
                                   + "Please inform RRDM" + Environment.NewLine
                        );
            ...
            string TotalProgressText = "MOVE TO MATCHED Cycle: ..."
