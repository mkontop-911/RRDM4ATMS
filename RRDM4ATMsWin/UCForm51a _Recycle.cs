...
{
    public partial class UCForm51a_Recycle : UserControl
    {
        // Stavros
     ...
     string WSearchP4;
        ...
        public void UCForm51a_Recycle_Par(string InSignedId, int InSignRecordNo, string InOperator, string InAtmNo, int InSesNo)
        {
       ...
       //     WUserOperator = Us.Operator;
            ...
            else
            {
                ...
                if (Ac.Supplier.Trim() == "Wincor")
                {
                    ...
                    if (SM.RecordFound == true)
                    {
                        ...
                        {
                            IsRecycle = true;
                            IsRecycleWincor = true;
                        }
                    }

                    if (IsRecycle == true)
                    {
                        textBoxMachineRecon.Hide();
                        panelMachineRecon.Hide();
                    }
                }
        ...
        public void SetScreen()
        {
            try
            {
                ...
                if (Na.BalSets >= 1)
                {
                    decimal DepToBeAdded = 0;
                    decimal TotalDebits = 0;
                    RRDMRepl_SupervisorMode_Details_Recycle SM = new RRDMRepl_SupervisorMode_Details_Recycle();
                    string Ccy = "EGP";
                    SM.Read_SM_AND_FillTable_Deposits_2_FOR_Ccy(WAtmNo, WSesNo, Ccy);
                    {
                        if (SM.RecordFound == true)
                        {
                            DepToBeAdded = SM.TotalNCR_DepositsDispensedMoney; 
                        }
                    }
                    ...
                    string WTableId = "Atms_Journals_Txns";
                     Mgt.ReadTrans_TXNS_FromSpecificTableBetween_Dates("", WTableId, WAtmNo,Ta.SesDtTimeStart, Ta.SesDtTimeEnd, 1, 2);
                    //  Mgt.TotalDebit;
                    TotalDebits = Mgt.TotalDebit;

                    labelCur1.Text = Na.Balances1.CurrNm; // First Currency  
                    textBoxOpenBal1.Text = (Na.Balances1.OpenBal+ DepToBeAdded).ToString("#,##0.00");
                    //  textBoxDispensed1.Text = ((Na.Balances1.OpenBal + DepToBeAdded) - Na.Balances1.ReplToRepl).ToString("#,##0.00");
                    textBoxDispensed1.Text = Mgt.TotalDebit.ToString("#,##0.00");
                    textBoxClosingCash1.Text = ((Na.Balances1.OpenBal + DepToBeAdded)- TotalDebits).ToString("#,##0.00");
                    textBoxMachineCounters1.Text = Na.Balances1.MachineBal.ToString("#,##0.00");
                    textBoxDifference1.Text = (((Na.Balances1.OpenBal + DepToBeAdded) - TotalDebits) - Na.Balances1.MachineBal).ToString("#,##0.00");

                    CycleDifference1 = (((Na.Balances1.OpenBal + DepToBeAdded) - TotalDebits) - Na.Balances1.MachineBal);
                }
                if (Na.BalSets >= 2)
                {
                ...
                else
                {
                    ...
                    // ***********************
                    // 
                    if (IsRecycleWincor == true)
                    {
                        // Dont show message for wincor
                    }
                    else
                    {
