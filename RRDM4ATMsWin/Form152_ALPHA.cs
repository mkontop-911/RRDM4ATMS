...
{
    public partial class Form152_ALPHA : Form
    {
        ...
        public Form152_ALPHA(string InSignedId, int InSignRecordNo, string InOperator, int Action)
        {
        ...
        private void dataGridViewMyATMS_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            labelATM_logical_ID.Text = Jd.AtmNo;
            ...
            Ta.ReadReplCyclesForFromToDateFillTable_Alpha(WOperator, WSignedId, WAtmNo, WDtFrom, WDtTo);
            ...
            //   // dataGridView2_RowEnter(this, new DataGridViewCellEventArgs(1, 1));
            //}
        ...
        private void dataGridView2_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //WRow1 = 0;
            //dataGridViewMyATMS.Rows[WRow1].Selected = true;
            //dataGridView2_RowEnter(this, new DataGridViewCellEventArgs(1, WRow1));
            ...
            else
            {
                ...
                string WSelectionCriteria = " WHERE AtmNo='"+ WAtmNo + "' AND ReplCycleNo =" + WSesNo;
                Cr.Read_CASH_RECON_And_BySelectionCriteria(WSelectionCriteria);

                if (Cr.RecordFound== true)
                {
                    // Fill in the Journal Fields 
                    //
                    // WITHDRAWLS
                    textBoxOpenBal_J.Text = Cr.SM_OpeningBalance.ToString("#,##0.00");
                    textBoxRemaining_J.Text = Cr.SM_Remaining.ToString("#,##0.00");
                    textBoxNewLoaded_J.Text = Cr.SM_Cash_Loaded.ToString("#,##0.00");
                    textBoxAddedCash_J.Text = Cr.SM_Cash_Loaded_Minus_SM_Remaining.ToString("#,##0.00"); // Calculated from Journal 
                    textBoxAddedCash_Counted_J.Text = Cr.ATM_Replenishment.ToString("#,##0.00"); // FOUND IN GL 

                    textBoxCassette_Excess_J.Text = Cr.OverFound_Cassettes.ToString("#,##0.00");
                    textBox_Cassette_Shortage_J.Text = Cr.ShortFound_Cassettes.ToString("#,##0.00");

                    // DEPOSITS 
                    textBoxDeposits_J.Text = Cr.SM_Deposits.ToString("#,##0.00");
                    textBoxDeposits_Counted_J.Text = Cr.ATM_Deposits.ToString("#,##0.00");

                    //textBox_Deposits_Excess_J.Text = Cr.OverFound_Deposits.ToString("#,##0.00");
                    //textBox_Deposits_Shortage_J.Text = Cr.ShortFound_Deposits.ToString("#,##0.00");

                    // Fill in the SWITCH Fields 
                    //
                    // WITHDRAWLS
                    textBoxOpenBal_S.Text = Cr.SWITCH_OpeningBalance.ToString("#,##0.00");
                    textBoxRemaining_S.Text = Cr.SWITCH_Remaining.ToString("#,##0.00");
                    textBoxNewLoaded_S.Text = Cr.SWITCH_Cash_Loaded.ToString("#,##0.00");
                    textBoxAddedCash_S.Text = Cr.SWITCH_Cash_Loaded_Minus_SWITCH_Remaining.ToString("#,##0.00"); // Calculated from Journal 

                    textBoxAddedCash_Counted_S.Text = Cr.ATM_Replenishment.ToString("#,##0.00"); // FOUND IN GL 

                    textBoxCassette_Excess_S.Text = Cr.OverFound_Cassettes_SWITCH.ToString("#,##0.00");
                    textBox_Cassette_Shortage_S.Text = Cr.ShortFound_Cassettes_SWITCH.ToString("#,##0.00");

                    textBoxInsuranceAmt.Text = Cr.InsuranceAmt.ToString("#,##0.00");
                    if ( Cr.IsWithinInsurance == true)
                    {
                        labelInsuranceAlert.Hide();
                    }
                    else
                    {
                        labelInsuranceAlert.Show();
                    }
                   
                    // DEPOSITS 
                        textBoxDeposits_S.Text = Cr.SWITCH_Deposits.ToString("#,##0.00");
                    textBoxDeposits_Counted_S.Text = Cr.ATM_Deposits.ToString("#,##0.00");

                    //textBox_Deposits_Excess_S.Text = Cr.OverFound_Deposits_SWITCH.ToString("#,##0.00");
                    //textBox_Deposits_Shortage_S.Text = Cr.ShortFound_Deposits_SWITCH.ToString("#,##0.00");

                    textBoxReplenished_GL.Text = Cr.ATM_Replenishment.ToString("#,##0.00");
                    textBoxDeposits_GL.Text = Cr.ATM_Deposits.ToString("#,##0.00");
                    textBox_DR_ATM_GL.Text = Cr.DB_Corrections_on_ATM.ToString("#,##0.00");
                    textBox_CR_ATM_GL.Text = Cr.CR_Corrections_on_ATM.ToString("#,##0.00");

                    textBoxPresenterError.Text = Cr.RRDM_PresentedErrors.ToString("#,##0.00");
                    textBoxSuspectAmt.Text = Cr.RRDM_SuspectFound.ToString("#,##0.00");

                    if (Cr.IsDifferent == true)
                    {
                        textBoxReplGenComment.Text = "THERE ARE DIFFERENCES"; 
                    }
                    else
                    {
                        textBoxReplGenComment.Text = "EVERYTHING IN AGREEMENT";
                    }
                ...
                {
                    // Initialise Journal 
                    textBoxOpenBal_J.Text = "0.00";
                    textBoxRemaining_J.Text = "0.00";
                    textBoxNewLoaded_J.Text = "0.00";
                    textBoxAddedCash_J.Text = "0.00";
                    textBoxAddedCash_Counted_J.Text = "0.00";


                    textBoxCassette_Excess_J.Text = "0.00";
                    textBox_Cassette_Shortage_J.Text = "0.00";

                    // DEPOSITS 
                    textBoxDeposits_J.Text = "0.00";
                    textBoxDeposits_Counted_J.Text = "0.00";

                    
                    // Initialise SWITCH 
                    textBoxOpenBal_S.Text = "0.00";
                    textBoxRemaining_S.Text = "0.00";
                    textBoxNewLoaded_S.Text = "0.00";
                    textBoxAddedCash_S.Text = "0.00"; 
                    textBoxAddedCash_Counted_S.Text = "0.00";

                    textBoxCassette_Excess_S.Text = "0.00";
                    textBox_Cassette_Shortage_S.Text = "0.00";

                    // DEPOSITS 
                    textBoxDeposits_S.Text = "0.00";
                    textBoxDeposits_Counted_S.Text = "0.00";

                    //textBox_Deposits_Excess_S.Text = "0.00";
                    //textBox_Deposits_Shortage_S.Text = "0.00";

                    textBoxReplenished_GL.Text = "0.00";
                    textBoxDeposits_GL.Text = "0.00";
                    textBox_DR_ATM_GL.Text = "0.00";
                    textBox_CR_ATM_GL.Text = "0.00";
                }
      ...
      // FOR CYCLE    
            ...
            // NOTES 3 START  
            Order = "Descending";
            WParameter4 = "REPL_Cycle_No:" + WSesNo;
            WSearchP4 = "";
            Cn.ReadAllNotes(WParameter4, WSignedId, Order, WSearchP4);
            if (Cn.RecordFound == true)
            {
        ...
        private void ShowGrid_ReplCycles()
        {
            ...
            if (Ta.ATMsReplCyclesSelectedPeriod.Rows.Count ==0)
            {
                WAtmNo = WAtmNo;
                MessageBox.Show("No Replenihment to Show");
           ...
           // scrollPosition = dataGridView2.FirstDisplayedScrollingRowIndex;
            // Load Grid 
        ...
        // SWITCH LINES
        private void linkLabelSWITCH_LINES_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            if (WSesNo == 0)
            {
                MessageBox.Show("Nothing to show");
            ...
            if (SM.RecordFound == true)
            {
                string WSelectionCriteria = " WHERE OriginSeqNo=" + SM.SeqNo;
                Sw.Read__SM_SWITCH_TXNS_And_BySelectionCriteria(WSelectionCriteria);
                ...
                if (Sw.RecordFound == true)
                {
                    ...
                    string WTraceRRNumber = "";
                    NForm67_BDC = new Form67_BDC(WSignedId, 0, WOperator, SM.Fuid, WTraceRRNumber, WAtmNo
                        , Sw.sessionstart_ruid, Sw.sessionend_ruid, NullPastDate, NullPastDate, Mode);
                    NForm67_BDC.Show();
                }
                else
                {
