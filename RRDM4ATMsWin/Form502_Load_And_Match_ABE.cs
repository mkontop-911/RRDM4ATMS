...
{
    public partial class Form502_Load_And_Match_ABE : Form
    {
        ...
        public Form502_Load_And_Match_ABE(string InSignedId, int SignRecordNo, string InOperator
        ...
        {
            dataGridViewFilesStatus.DataSource = Lf_ABE.SourceFilesTotals.DefaultView;
                    ...
                    foreach (string file in allJournals)
                    {
                        ...
                        if (ext == ".log" || ext == ".LOG")
                        {
                            try
                            {
                                ...
                                string WType = OldFileName.Substring(19, 6);

                                string NewFileName = "";
                                bool ValidName = false;
                                if (WType == "_EJ_NC" || WType == "_EJ_DN")
                                {
                                    // OK 
                                    ValidName = true;
                                }
                                ...
                                {
                                    // Invalid Name 
                                    MessageBox.Show("Invalid Journal Name" + Environment.NewLine
                                        + OldFileName
                                        );
                                    ValidName = false;
                                }
                                ...
                                if (ValidName == true)
                                {
                                    if (WType == "_EJ_NC")
                                    {
                                        NewFileName = WATM + "_" + WYYYYY + WMM + WDD + "_EJ_NCR.001";
                                    }

                                    if (WType == "_EJ_DN")
                                    {
                                        NewFileName = WATM + "_" + WYYYYY + WMM + WDD + "_EJ_WCR.001";
                                    }
        ...
        private void buttonDoMatching_Click(object sender, EventArgs e)
        {
                          ...
                          == DialogResult.Yes)
            {
                ...
                Lf_ABE.DeleteRecordsToSetStartingPoint(WOperator, DeleteDate, WCut_Off_Date, WReconcCycleNo);
            }
            else
            {
                // Stop 
            ...
            Lf_ABE.UpdateRecordsWithTraceAndOther(WOperator, WReconcCycleNo, 0, 2, WCut_Off_Date);
            ...
            Lf_ABE.UPDATE_Mpa_After_Matching_With_ReplCycle(WOperator, WReconcCycleNo);
        ...
        private void buttonLoadFiles_Click(object sender, EventArgs e)
        {
            ...
            //
            //****************************
            string Temp = "No_Panicos"; //  Check IST 
            if (Temp == "Panicos")
            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            //
            // UNDO STATISTICS 
            Lf_ABE.HandleDailyStatisticsForAtms_UNDO(WReconcCycleNo);

            // UNDO Intbl_CIT_Bank_Repl_Entries and Posted Txns ( BOTH ) 
            // Lf_ABE.HandleSM_Bank_Records_UNDO(WReconcCycleNo);
            //
        ...
        private void METHOD_LoadFiles()
        {
            ...
            try
            {
                    ...
                    if (IsPresent == true & IsGood == "YES" & SourceFileId != "Atms_Journals_Txns")
                    {
                        ...
                        if (Environment.UserInteractive)
                        {
                            if (IsGood == "YES")
                            {
                                ...
                                Lf_ABE.InsertRecordsInTableFromTextFile_InBulk(WOperator, SourceFileId, FullFileName, Ms.InportTableName, Ms.Delimiter, FlogSeqNo);
                                ...
                                Flog.LineCount = Lf_ABE.stpLineCount;

                                Flog.stpReturnCode = Lf_ABE.stpReturnCode;
                                Flog.stpErrorText = Lf_ABE.stpErrorText;
                                Flog.stpReferenceCode = Lf_ABE.stpReferenceCode;

                                if (Lf_ABE.stpReturnCode == 0)
                                ...
                                int r = Lf_ABE.stpReturnCode;
                                string D = Lf_ABE.stpErrorText;
                            }
                ...
                Lf_ABE.UpdateRecordsWithTraceAndOther(WOperator, WReconcCycleNo, FlogSeqNo, 1, WCut_Off_Date);
                //
                // EXTRA FIELDS
                //MessageBox.Show("We Start extra fields");
                Lf_ABE.UpdateFiles_With_EXTRA(WOperator, WReconcCycleNo);
        ...
        {
            Lf_ABE.GetTotals(WOperator, WReconcCycleNo);
...
// Create Visa File 
        private void buttonCreateVisaFile_Click(object sender, EventArgs e)
        {
            // CALL STORE PROCEDURE TO UNDO THE REST
            // THESE ARE FILES LIKE Reconciliation Sessions etc. 
            ...
            string InFullPath = "C:\\RRDM\\FilePool\\VISA_CARD\\EP745.txt";

            string OutPath = "C:\\RRDM\\FilePool\\VISA_CARD\\temp\\"; 
            ...
            string SPName = "[ATM_MT_Journals_AUDI].[dbo].[stp_00_RUN_PROCESS_VISA_FILE]";
...
//                    ALTER PROC[dbo].[stp_00_RUN_PROCESS_VISA_FILE]
//        (
//@FullPath AS NVARCHAR(254),
//	   @OutputFullPath AS NVARCHAR(254) 
// 	  @ReturnCode AS INT OUTPUT,
//      @ErrorText as NVARCHAR(2000) OUTPUT,  
//	  @ErrorReference as NVARCHAR(100)  OUTPUT
...
//--	 -- @BranchNo AS NVARCHAR(50),
//-- declare @FullPath   NVARCHAR(254) = 'C:\RRDM\FilePool\Atms_Journals_Txns\EP745.txt'
//-- declare @ErrorText as NVARCHAR(2000)
//--  declare @OutputFullPath   NVARCHAR(254)  = 'C:\temp\'
//--declare @ReturnCode   INT


                // the first are input parameters

                   cmd.Parameters.Add(new SqlParameter("@FullPath", InFullPath));
                   cmd.Parameters.Add(new SqlParameter("@OutputFullPath", OutPath));

                    // the following are output parameters
                    SqlParameter retOutputFileName = new SqlParameter("@OutputFileName", OutputFileName);
                    retOutputFileName.Direction = ParameterDirection.Output;
                    retOutputFileName.SqlDbType = SqlDbType.NVarChar;
                    retOutputFileName.Size = 3000;
                    cmd.Parameters.Add(retOutputFileName);
                    ...
                    SqlParameter retProgressText = new SqlParameter("@ErrorText", ProgressText);
                    retProgressText.Direction = ParameterDirection.Output;
                    retProgressText.SqlDbType = SqlDbType.NVarChar;
                    retProgressText.Size = 3000;
                    cmd.Parameters.Add(retProgressText);
                    ...
                    ret = (int)cmd.Parameters["@ReturnCode"].Value;
                    ProgressText = (string)cmd.Parameters["@ErrorText"].Value;
                    OutputFileName = (string)cmd.Parameters["@OutputFileName"].Value;
                    ...
                    {
                        // OK
                        MessageBox.Show("Visa File Has Been Created With name :"+ OutputFileName);
                    }
                    else
                    {
                        MessageBox.Show("Visa File not Created"); 
                    }
