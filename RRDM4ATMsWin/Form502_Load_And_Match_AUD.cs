...
{
    public partial class Form502_Load_And_Match_AUD : Form
    {
        ...
        RRDMMatchingOfTxns_V02_MinMaxDt_BDC_4_ROM Mt = new RRDMMatchingOfTxns_V02_MinMaxDt_BDC_4_ROM();
        ...
        public Form502_Load_And_Match_AUD(string InSignedId, int SignRecordNo, string InOperator
        ...
        private void buttonLoadFiles_Click(object sender, EventArgs e)
        {
           ...
           // RRDMJournalRead_HstAtmTxns_AndCreateTable_V2_With_SM_BDC_2_Pambos2 PambosLoad = new RRDMJournalRead_HstAtmTxns_AndCreateTable_V2_With_SM_BDC_2_Pambos2();
            RRDMJournalRead_HstAtmTxns_AndCreateTable_V2_With_SM_BDC_2 FAB_LOAD = new RRDMJournalRead_HstAtmTxns_AndCreateTable_V2_With_SM_BDC_2();
            ...
            {
                // WAITING FOR PAMBOS TO CORRECT the AUTH and duplicates with OK
               Mpa.DeleteDuplicates_tblHstAtmTxnsROM_EURO(WReconcCycleNo); 
                RRDM_Journal_TransactionSummary_V2 Ts = new RRDM_Journal_TransactionSummary_V2();
                int TempCycle = 200;
                Ts.Update_NULL_Values_InTxnBalances(TempCycle);

                RRDMJournalRead_HstAtmTxns_AndCreateTable_V2_With_SM_BDC_2_Pambos2 PambosLoad = new RRDMJournalRead_HstAtmTxns_AndCreateTable_V2_With_SM_BDC_2_Pambos2();
                // GET FROM PAMBOS TO MASTER
                
                PambosLoad.ReadJournal_Txns_And_Insert_In_Pool_ROM(WSignedId, WSignRecordNo, WOperator, WReconcCycleNo, IsRomaniaVersion);

            }
            else
            {
                FAB_LOAD.ReadJournal_Txns_And_Insert_In_Pool(WSignedId, WSignRecordNo, WOperator, 0, "", 0, 4);
            }
            ...
            if (Is_SuperisorMode == true)
            {
                ...
                if (IsRomaniaVersion == true)
                {
                    RRDMRepl_SupervisorMode_Master_ROM Smaster = new RRDMRepl_SupervisorMode_Master_ROM();
                    int Sm_Mode = 3;
            ...
            if (Gp.RecordFound & Gp.OccuranceNm == "YES")
            {
                ...
                Mpa.DeleteDuplicates_tblHstAtmTxnsROM(WReconcCycleNo);
            }
            ...
            OccurId = "2"; // 
            Gp.ReadParametersSpecificId(WOperator, ParId, OccurId, "", "");
            if (Gp.RecordFound & Gp.OccuranceNm == "YES" & IsRomaniaVersion ==false)
            {
                ...
                // ALSO WE CREATE TRANSACTIONS IF THE PARAMETER IS SET t
                    ...
                    if (IsPresent == true & IsGood == "YES" & SourceFileId != "Atms_Journals_Txns")
                    {
                        ...
                        if (Environment.UserInteractive)
                        {
                            if (IsGood == "YES")
                            {
                                ...
                                if (IsRomaniaVersion == true)
                                {
                                    Lf_ROM.InsertRecordsInTableFromTextFile_InBulk(WOperator, SourceFileId, FullFileName, Ms.InportTableName, Ms.Delimiter, FlogSeqNo);

                                    Flog.ReadLoadedFilesBySeqNo(FlogSeqNo);

                                    Flog.StatusVerbose = "";

                                    Flog.LineCount = Lf_ROM.stpLineCount;

                                    Flog.stpReturnCode = Lf_ROM.stpReturnCode;
                                    Flog.stpErrorText = Lf_ROM.stpErrorText;
                                    Flog.stpReferenceCode = Lf_ROM.stpReferenceCode;
                                ...
                                if ((Lf_AUD.stpReturnCode == 0 & IsRomaniaVersion==false) 
                                    || (Lf_ROM.stpReturnCode == 0 & IsRomaniaVersion == true ) )
                                {
                                    Mcf.UpdateReconcCategoryVsSourceRecordProcessCodeToMinusOne(SourceFileId, WReconcCycleNo);
