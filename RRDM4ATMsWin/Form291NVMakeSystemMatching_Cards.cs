...
{
    public partial class Form291NVMakeSystemMatching_Cards : Form
    {
        ...
        RRDMNVMakeMatchingAndAlertsClass_Cards Mmc = new RRDMNVMakeMatchingAndAlertsClass_Cards();
        ...
        public Form291NVMakeSystemMatching_Cards(string InSignedId, int SignRecordNo, string InOperator, string InSubSystem)
        {
            ...
            if (WSubSystem == "CardsSettlement")
            {
                ...
                WOrigin = "Master Card";  // Used For Categories 
            }
            if (WSubSystem == "E_FINANCE")
            {

                WJobCategory = "E_FINANCE Reconciliation";
                WOrigin = "E_FINANCE"; // Used For Categories 
            ...
            //
            WDateOfCycle = DateTime.Now.Date; 
            ...
            if (Rjc.RecordFound == true)
            {
                ...
                //WDateOfCycle = Rjc.StartDateTm.Date;

                if (Rjc.StartDateTm.Date == WDateOfCycle.Date)
                {
                    WReconcCycleNo = Rjc.JobCycle;
                    MessageBox.Show("Daily Cycle for.."+ WJobCategory + "..Already Opened" + Environment.NewLine
                                              //    + "Run Refresh Data to restart"          
                                                    );
                }
                else
                {
                    // Insert new Cycle 
                    WDateOfCycle = DateTime.Now;
                    ...
                    else Rjc.Description = "E_FINANCE Reconciliation";
            ...
            else
            {
                ...
                if (WJobCategory == "CardsSettlement")
                           Rjc.Description = "Visa Settlement to Authorisation Reconciliation";
                else Rjc.Description = "E_FINANCE Reconciliation";
            ...
            //
            // TRNS OF ALL CATEGORIES ARE INSERTED IN INTERNAL AND EXTERNAL FILES
            // THE MATCHING IS DONE BY CATEGORY
            // THE TXNS ARE MATCHED BASED ON THE INTERNAL AND EXTERNAL ACCOUNT FOR EACH CATEGORY
            //
            ...
            // STEP C - Load Rates 
            //***************************************************
            // Update VISA Authorisation Txns with
            // Rates 

            Sec.ReadInternalAndUpdateCurrentRates(WOperator);
            ...
            //        - NOTE that the ones without tolerance were matched in the previous step 
            //******************************************************************
            ...
            //***************************************************
            // Identify and Mark Alerts For the Unmatched Internal And External
            // Update Reconciliation Category Session 

            // For Cards Identify unmatched Transactions over certain period defined in parameters 
            // Make this transaction Matched with reason .... 
            ...
            // Totals 
            Rcs.UpdateReconciliationSessionsForCardsForTotals(WOperator, WSignedId,
                                              WReconcCycleNo, WSubSystem,
                                              WDateOfCycle);
        ...
        private void Form291_Load(object sender, EventArgs e)
        {
            ...
            if (comboBoxFilter.Text == "Matched_This_Cycle")
            {
                if (WSubSystem == "E_FINANCE")
                {
                    if (WUserRole == "05")
                    {
                        ...
                        OrderCriteria = " CategoryId,  Origin, TxnAmt ASC";
                        Sec.ReadNVCardRecordsForBothByMode(WOperator, WSignedId, WSubSystem, Mode, WReconcCycleNo, "", "", OrderCriteria, WDateOfCycle, "");

                    }
                    if (WUserRole == "04")
                    {
                        Mc.ReadMatchingCategorybyActiveCategId(WOperator, "BDC" + WBranch); 
                        Mode = 1; // All auto matched this cycle 
                        OrderCriteria = " CategoryId,  Origin, TxnAmt ASC";
                        Sec.ReadNVCardRecordsForBothByMode(WOperator, WSignedId, WSubSystem, Mode, WReconcCycleNo, Mc.VostroAcc, Mc.InternalAcc, OrderCriteria, WDateOfCycle, "");
                    }
                }
                ...
                {
                    Mode = 9; // All auto matched this cycle 
                    Sec.ReadNVCardRecordsForBothByMode(WOperator, WSignedId, WSubSystem, Mode, WReconcCycleNo, "", "", "", WDateOfCycle, "");
                }
            ...
            if (comboBoxFilter.Text == "All Un Matched")
            {
                if (WSubSystem == "E_FINANCE")
                {
                    ...
                    {
                        Mc.ReadMatchingCategorybyActiveCategId(WOperator, "BDC" + WBranch);
                        Mode = 3; // All auto matched this cycle 
                        OrderCriteria = " CategoryId,  Origin, TxnAmt ASC";
                        Sec.ReadNVCardRecordsForBothByMode(WOperator, WSignedId, WSubSystem, Mode, WReconcCycleNo, Mc.VostroAcc, Mc.InternalAcc, OrderCriteria, WDateOfCycle, "");
                    }
                }
        ...
        private void dataGridView1_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            textBox1.Text = Sec.UniqueMatchingNo.ToString();
            textBox2.Text = Sec.TxnValueDate.ToString();
            textBox3.Text = Sec.TxnAmt.ToString("#,##0.00");
...
// GridCreation
        private void GridCreation()
        {
            ...
            dataGridView1.Columns[2].Width = 60; // Category Id
            ...
            dataGridView1.Columns[4].Width = 100; //MatchedType
            ...
            dataGridView1.Columns[13].Width = 110; // RRN
            ...
            dataGridView1.Columns[14].Width = 110; // OtherDetails
        ...
        public void ShowGrid11()
        {
            ...
            if (dataGridView1.Rows.Count == 0)
            {
                if (comboBoxFilter.Text == "Alerts")
                {
                    ...
                    Form2 MessageForm = new Form2("No Alerts This Cycle");
                    MessageForm.ShowDialog();
                ...
                if (comboBoxFilter.Text == "All Un Matched")
                {
                    ...
                    Form2 MessageForm = new Form2("No UnMatched This Cycle");
                    MessageForm.ShowDialog();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string P1 = "SETTLEMENT REPORT OF : " + comboBoxFilter.Text;
            ...
            Form56R66VISA ReportNOSTRO64 = new Form56R66VISA(P1, P2, P3, P4, P5, P6, P7);
            ReportNOSTRO64.Show();
        }
