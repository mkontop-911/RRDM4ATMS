...
{
    public partial class UCForm276c_AUDI_FirstStep : UserControl
    {
    ...
    //    RRDMReplOrdersClass Ro = new RRDMReplOrdersClass();
        ...
        public void UCForm276c_AUDI_FirstStep_Par(string InSignedId, int InSignRecordNo, string InOperator,
                                           string InCitId, int InLoadingCycle, int InRMCycle)
        {
            ...
            {
                Aoc.DeleteOccurancesByLoadingExcelCycleNo(WLoadingCycle, "FAB_FIRST_STEP"); 
                // CREATE TXNS 
                Create_TXNS_For_Validated_Load();

                Create_TXNS_For_Validated_UN_Load();
            }   
            //
            //************************************************************
        ...
        public void SetScreen()
        {
            ...
            if (WRequestor == true) // Comes from Author management Requestor
            {
                ...
                if (Ap.RecordFound == true & Ap.OpenRecord == true)
                {
                    ...
                    //labelAuthComm
                    //textBoxComment
                    if (Ap.Stage == 1 || Ap.Stage == 2 || Ap.Stage == 3)
                    {
                        // 2= Authorizer had receieved a mmessage , 
                        // 3=Authorizer took action, 
                        if (Ap.Stage == 1)
                        {
                            labelAuthStatus.Text = "AuthStatus: Authoriser Didnt Read the message yet";
                        }

                        if (Ap.Stage == 2)
                        {
                            labelAuthStatus.Text = "AuthStatus: Authorizer had receieved a mmessage";
                        }

                        if (Ap.Stage == 3)
                        {
                            labelAuthStatus.Text = "AuthStatus: Authorizer took action";
                        }
        ...
        private void Create_TXNS_For_Validated_Load()
        {
            ...
            // Make Selection Of Validated Entries FOR LOADED BASED ON THE Repl_Load_Status 
            int TempMode = 1;
            ...
            while (I <= (G4.DataTableG4SEntries.Rows.Count - 1))
            {
                ...
                WSesNo = (int)G4.DataTableG4SEntries.Rows[I]["ReplCycleNo"];
                ...
                if (Ta.ProcessMode == 2)
                {
                    ...
                    MessageBox.Show("Replenishment for ATM :" + WAtmNo + Environment.NewLine
                     + " And Repl Cycle : " + WSesNo.ToString() + Environment.NewLine
                      ...
                      + " Was Made on :  " + Ta.Repl1.ReplFinDtTm.ToString()
                ...
                if (WCash_Loaded_CIT > 0)
                {
                    // Create transactions 

                    CreateActions_Occurances_Load(WAtmNo, WSesNo, WCash_Loaded_CIT);

                }

                // UPDATE ProcessMode_Load = 1 
                int TempProcessMode_Load = 1; 
                    
                G4.UpdateCIT_G4S_Repl_EntriesProcessMode_Load(WSeqNo, TempProcessMode_Load, TempMode); 
        ...
        private void Create_TXNS_For_Validated_UN_Load()
        {
            // Update RRDM Replenishmet record
            //
            RRDM_CIT_G4S_And_Bank_Repl_Entries G4 = new RRDM_CIT_G4S_And_Bank_Repl_Entries();
            ...
            // Make Selection Of Validated Entries FOR UN LOADED 
            int TempMode = 1;
            string SelectionCriteria = " WHERE CITId = '" + WCitId + "' AND Repl_UnLoad_Status in ( 4,6 ) AND ProcessMode_UnLoad <> 2 AND PresentedErrors = 0 ";
            ...
            while (I <= (G4.DataTableG4SEntries.Rows.Count - 1))
            {
                ...
                if (
                    (WProcessMode_Load == 1 || WProcessMode_Load == 2) & WRepl_Load_Status == 4
                    & (WProcessMode_Load == 1 & WRepl_UnLoad_Status == 4 & WPresentedErrors == 0)
                    )
                {
                    GoodForAuto = true;
                }

                if ((WUnloadedCounted + WDeposits) > 0 & GoodForAuto == true)
                {
                   CreateActions_Occurances_Un_Load(WAtmNo, WSesNo, WCash_Loaded_CIT, WUnloadedCounted, WDeposits);
                }


                // UPDATE LOADING excel CYCLE for each ATM 

                // UPDATE ProcessMod_Unload = 1 


                int TempProcessMode_UnLoad = 1;

                    G4.UpdateCIT_G4S_Repl_EntriesProcessMode_UnLoad(WSeqNo, TempProcessMode_UnLoad, TempMode);
        ...
        public void CreateActions_Occurances_Load(string InAtmNo, int InSesNo, decimal InCashInAmount)
        {
            RRDMAtmsClass Ac = new RRDMAtmsClass();
            ...
            if (HybridRepl == false)
            {
                ...
                // FIRST DOUBLE ENTRY 
                WActionId = "14"; // 14_DR Intermediate/CR Speed Service

                WUniqueRecordId = WWSesNo; // SesNo 
                WCcy = "EGP";
                DoubleEntryAmt = InCashInAmount;
                string WMaker_ReasonOfAction = "Controller_Feeding ATM_First_Leg";
                Aoc.CreateActionsTxnsPerActionId(WOperator, WSignedId,
                                                      WActionId, WUniqueRecordIdOrigin,
                                                      WUniqueRecordId, WCcy, DoubleEntryAmt, WWAtmNo, WWSesNo
                                                      , WMaker_ReasonOfAction, "Replenishment");
                ...
                Aoc.UpdateOccurancesLoadingExcelCycleNo(WWAtmNo, WWSesNo, WActionId, WLoadingCycle);
        ...
        public void CreateActions_Occurances_Un_Load(string InAtmNo, int InSesNo, decimal InCash_Loaded_CIT, decimal InUnloadedCounted, decimal InDeposits)
        {
            RRDMAtmsClass Ac = new RRDMAtmsClass();
            ...
            if (HybridRepl == false)
            {
                ...
                WUniqueRecordId = InSesNo; // SesNo 
                WCcy = "EGP";
                DoubleEntryAmt = InCash_Loaded_CIT;
                string WMaker_ReasonOfAction = "Controller_Load ATM-Second Leg";
                Aoc.CreateActionsTxnsPerActionId(WOperator, WSignedId,
                                                      WActionId, WUniqueRecordIdOrigin,
                                                      WUniqueRecordId, WCcy, DoubleEntryAmt, InAtmNo, InSesNo
                                                      , WMaker_ReasonOfAction, "Replenishment");
                ...
                Aoc.UpdateOccurancesLoadingExcelCycleNo(InAtmNo, InSesNo, WActionId, WLoadingCycle);


                // Unload 
                // decimal InUnloadedCounted, decimal InDeposits
                DoubleEntryAmt = InUnloadedCounted + InDeposits; 
                ...
                // InUnloadedCounted, decimal InDepoits
                DoubleEntryAmt = DoubleEntryAmt;
                WMaker_ReasonOfAction = "Controller_DR CIT";
                Aoc.CreateActionsTxnsPerActionId(WOperator, WSignedId,
                                                      WActionId, WUniqueRecordIdOrigin,
                                                      WUniqueRecordId, WCcy, DoubleEntryAmt, InAtmNo, InSesNo
                                                      , WMaker_ReasonOfAction, "Replenishment");
                ...
                WCcy = "EGP";
                DoubleEntryAmt = DoubleEntryAmt;
                WMaker_ReasonOfAction = "Controller_CR ATM CASH";
                Aoc.CreateActionsTxnsPerActionId(WOperator, WSignedId,
                                                      WActionId, WUniqueRecordIdOrigin,
                                                      WUniqueRecordId, WCcy, DoubleEntryAmt, InAtmNo, InSesNo
                                                      , WMaker_ReasonOfAction, "Replenishment");
        ...
        private void ShowAuthorisationInfo()
        {
            if (WViewFunction == true)
            {
                ...
                Ap.ReadAuthorizationForReplenishmentReconcSpecificForCategoryAndCycleVIEW_close(WCategoryId, WLoadingCycle, "LoadingExcel"); 
            }
            else
            {
                // Open Record
        ...
        private void TotalsAfterValidation()
        {
            ...
            int TempMode = 1;
            //string SelectionCriteria = " WHERE CITId = '" + WCitId + "' AND LoadingExcelCycleNo =" + WRMCycle;
            string SelectionCriteria = " WHERE CITId = '" + WCitId + "' AND (Repl_Load_Status = 4)"
                + " AND Cash_Loaded > 0 ";
            ...
            textBoxTotalInLines.Text = G4.WTotalEntries.ToString();
            textBoxTotalAmt.Text = G4.WTotalLoaded.ToString("#,##0.00");
            textBoxTotalPairs.Text = G4.WTotalEntries.ToString();
