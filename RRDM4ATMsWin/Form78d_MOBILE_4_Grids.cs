...
{
    public partial class Form78d_MOBILE_4_Grids : Form
    {
        ...
        public Form78d_MOBILE_4_Grids(string InOpertor , string InSignedId, string InMatchingCateg, 
                                               int InRMCycle, int InMode, string InApplication)
        {
            ...
            labelWhatGrid.Text = "ANALYSIS BY TRANS TYPE FOR MATCHING CATEGOR.."+ WMatchingCateg + " AT_CYCLE_" + WRMCycle.ToString();
        ...
        private void Form78b_Load(object sender, EventArgs e)
        {
            ...
            Mmob.ReadTablebyCycleAndCategory(WMatchingCateg, WRMCycle, W_Application);

            // Grid 1 
            
            if (Mmob.MatchingCategByTransType_A.Rows.Count>0)
            {
                dataGridView1.DataSource = Mmob.MatchingCategByTransType_A.DefaultView;
                ShowGrid_1();

                labelFILE_A.Text = "FILE.." + Mmob.FileA + "..Matched + Unmatched";

                textBoxTotalAmt_1.Text = Mmob.TotalAmount_1.ToString("#,##0.00");
                textBoxTXNS_1.Text = Mmob.TotalNumber_1.ToString("#,##0");
            }
            else
            {
            ...
            if (Mmob.MatchingCategByTransType_B.Rows.Count > 0)
            {
                dataGridView2.DataSource = Mmob.MatchingCategByTransType_B.DefaultView;

                ShowGrid_2();

                labelFile_B.Text = "FILE.." + Mmob.FileB + "..Matched + Unmatched";

                textBoxTotalAmt_2.Text = Mmob.TotalAmount_2.ToString("#,##0.00");
                textBoxTXNS_2.Text = Mmob.TotalNumber_2.ToString("#,##0");
            }
            else
            {
            ...
            if (Mmob.MatchingCategByTransType_C.Rows.Count > 0)
            {
                dataGridView3.DataSource = Mmob.MatchingCategByTransType_C.DefaultView;
                ShowGrid_3();

                labelFILE_A_U.Text = "FILE.." + Mmob.FileA + "..Unmatched Mask exist in A  ";

                textBoxTotalAmt_3.Text = Mmob.TotalAmount_3.ToString("#,##0.00");
                textBoxTXNS_3.Text = Mmob.TotalNumber_3.ToString("#,##0");
            }
            else
            {
            ...
            if (Mmob.MatchingCategByTransType_D.Rows.Count > 0)
            {
                dataGridView4.DataSource = Mmob.MatchingCategByTransType_D.DefaultView;
                ShowGrid_4();

                labelFile_B_U.Text = "FILE.." + Mmob.FileB + "..Unmatched exist in B ";

                textBoxTotalAmt_4.Text = Mmob.TotalAmount_4.ToString("#,##0.00");
                textBoxTXNS_4.Text = Mmob.TotalNumber_4.ToString("#,##0");
            }
            else
            {
