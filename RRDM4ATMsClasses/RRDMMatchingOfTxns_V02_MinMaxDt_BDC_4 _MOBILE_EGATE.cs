...
namespace RRDM4ATMs
{
    public class RRDMMatchingOfTxns_V02_MinMaxDt_BDC_4_MOBILE_EGATE : Logger
    {
        public RRDMMatchingOfTxns_V02_MinMaxDt_BDC_4_MOBILE_EGATE() : base() { }
        /// <summary>
        ...
        //readonly string connectionStringRec = AppConfig.Configuration.GetConnectionString
       ...
       // RRDMMatchingTxns_MasterPoolATMs Mpa = new RRDMMatchingTxns_MasterPoolATMs();
        ...
        string SourceTable_A_Origin;
        string SourceTable_B_Origin;
        string SourceTable_C_Origin;
                                       ...
                                       int InRMCycle, bool InTestingWorkingFiles)
        {
            ...
            if (Mcsf.SourceFileNameA == "TXNS_MASTER"
            ...
            if (NumberOfTables == 2 & IsFrom_Master == false)
            {
                ...
                {
                    TempMaster = W_DB + ".[dbo].[TXNS_MASTER]";
                }
                
                // MOVE TABLEA TO MASTER
                M_EGATE.ReadUniversalAndCreateMaster_Mobile_Records(WOperator, TempMaster, SourceTable_A, WMatchingCateg, WRMCycle, W_Application);

                // Change SourceTable_A to MASTER 
                SourceTable_A_Origin = SourceTable_A; // Keep it 
            ...
            RecordsToMatch = true; // left here on purpose 
        ...
        private void MakeMatchingByGroupOrCategory(string InOperator, string InSignedId, string InMatchingCateg, string InReconcCateg, int InRMCycle, int InAtmsReconcGroup)
        {
            ...
            if (NumberOfTables == 2 & POS_Type == false)
            {
                ...
                {
                    // Latest way
                    // Working files are created and then matching is done 
                                      ...
                                      int InRMCycle)
        {
            ...
            TableId = SourceTable_A;
            M_EGATE.UpdateSourceTablesAsProcessed_MOBILE_Based_ON_ALL(WMatchingCateg, TableId, WRMCycle, MatchMask);
       ...
       + " ";
                        ...
                        new SqlCommand(SqlString, conn))
                    {
                        ...
                        while (rdr.Read())
                        {
                            ...
                            if (InMatchingCateg == "EGA375")
                            {
                                ...
                                SecondTransAmount = M_EGATE.ReadTransactionAmountFromTable(SelectionCriteria, InFileIdB, W_Application);
        ...
        private void InsertAndUpdateTablesFromOurAtms_AND_JCC(string WWMask)
        {
            ...
            try
            {
                ...
                bool Previous = false;
                bool Is_DiffAmt_EGA375 = false;
                decimal DifAmt_EGA375 = 0;
                decimal FirstAmt = 0;
                decimal SecondAmt=0;
                // 
                if (WWMask == "10")
                {
                    if (WWMask.Substring(0, 1) == "1")
                    {
                        ...
                        M_EGATE.ReadTransSpecificFromSpecificTable_MOBILE(SelectionCriteria, TableId, 1, W_Application);

                        
                        // The record is read

                        if (MatchingCateg == "EGA375")
                        {
                            // Keep the TransAmount of the first record
                            FirstAmt = M_EGATE.TransAmount; 
                            int Db_Mode = 1; // look only the unmatched and reversals
                            SelectionCriteria = " WHERE MatchingCateg = '" + WMatchingCateg + "'"
                                    + " AND RRNumber ='" + M_EGATE.RRNumber + "'"
                                    + " AND LoadedAtRMCycle =" + WRMCycle                              
                                    ;
                            // Check if 0 record came eg yesterday 

                            M_EGATE.ReadTransSpecificFromSpecificTable_MOBILE(SelectionCriteria, SourceTable_B, Db_Mode, W_Application);
                            if (M_EGATE.RecordFound)
                            {
                                Is_DiffAmt_EGA375 = true;
                                SecondAmt = M_EGATE.TransAmount;
                                DifAmt_EGA375 = FirstAmt - SecondAmt; 
                            }
                        }
                        else
                        {
                            ...
                            SelectionCriteria = " WHERE MatchingCateg = '" + WMatchingCateg + "'"
                                    + " AND RRNumber ='" + M_EGATE.RRNumber + "'"
                                    + " AND TransAmount =" + M_EGATE.TransAmount
                            ...
                            M_EGATE.ReadTransSpecificFromSpecificTable_MOBILE(SelectionCriteria, SourceTable_B, Db_Mode, W_Application);
                            if (M_EGATE.RecordFound == true)
                            {
                                // Check if reversal 
                                // Check if done before with Mask 01
                                if (M_EGATE.Matched == false & M_EGATE.LoadedAtRMCycle < WRMCycle)
                                {
                                    // Previous day we had the record as 01
                                    Previous = true;
                                    M_EGATE.MatchMask = "N1"; // we update the old record as N1 ... came as Next day 
                                    M_EGATE.Comments = "Matching Record Came next cycle";
                                    M_EGATE.UpdateRecordAsUnmatchedBySeqNumber_MOBILE(SourceTable_B, M_EGATE.SeqNo);
                                }
                                if (M_EGATE.IsReversal == true & M_EGATE.LoadedAtRMCycle == WRMCycle)
                                {
                                    IsReversal = true;
                                }
                        ...
                        M_EGATE.IsMatchingDone = true;
                        M_EGATE.Matched = false;
                        M_EGATE.MatchMask = WWMask;
                        if (IsReversal == true)
                        {
                            M_EGATE.MatchMask = "1R";
                            M_EGATE.Comments = "Record is reversed ";
                        }
                        else
                        {
                            if (Previous == true)
                            {
                                M_EGATE.Comments = "Matched record appeared previous cycle";
                            }
                        }

                        if (Is_DiffAmt_EGA375 == true)
                        {
                            M_EGATE.Comments = "The Agent.'"+M_EGATE.RRNumber+"' Total differs"+Environment.NewLine
                                + "--------------------------" + Environment.NewLine
                                + "Salary Total.:" + FirstAmt.ToString("#,##0.000")+Environment.NewLine
                                + "Agent Total..:" + SecondAmt.ToString("#,##0.000") + Environment.NewLine
                                 + "----------------------------------" + Environment.NewLine
                                 + "----------------------------------" + Environment.NewLine
                                + "Difference...:" + DifAmt_EGA375.ToString("#,##0.000");
                            ; 
                        }

                        M_EGATE.MatchingAtRMCycle = WRMCycle;

                        M_EGATE.UpdateRecordAsUnmatchedBySeqNumber_MOBILE(TableId, SeqNoInOne);
                    ...
                    if (W_Application == "EGATE")
                    {
                        TempFrom = "[EGATE].[dbo]." + "TXNS_MASTER"; // By default we move them to master
                        TempFileA = "[EGATE].[dbo]." + TempFileA;
                        TempFileB = "[EGATE].[dbo]." + TempFileB;
                    ...
                    // & MatchingCateg != "ETI350" & MatchingCateg != "QAH350" & MatchingCateg != "IPN350")
                    {
                        ...
                        if (M_EGATE.RecordFound == true)
                        {
                            SelectionCriteria = " WHERE MatchingCateg = '" + WMatchingCateg + "'"
                                ...
                                + " AND TransAmount =" + M_EGATE.TransAmount
                                ;
                            M_EGATE.ReadTransSpecificFromSpecificTable_MOBILE(SelectionCriteria, TempFileA, 1, W_Application);
                            if (M_EGATE.RecordFound == true)
                            {
                                M_EGATE.IsMatchingDone = true;
                                M_EGATE.Matched = false;
                                ...
                                M_EGATE.UpdateRecordAsUnmatchedBySeqNumber_MOBILE(TempFileA, M_EGATE.SeqNo);
                            }
                        }
                ...
                if (WWMask == "01")
                {
                    ...
                    if (M_EGATE.RRNumber == "00015671-1526-5869-0111-7c1e08206594" || M_EGATE.RRNumber == "0015b703-e161-44d2-96bc-bc0ef8a0e4f5")
                    {
                        M_EGATE.RRNumber = M_EGATE.RRNumber;
                    }
                    ...
                    if (M_EGATE.RecordFound == true)
                    {
                        ...
                        M_EGATE.ReadTransSpecificFromSpecificTable_MOBILE(SelectionCriteria, SourceTable_A, Db_Mode, W_Application);
                        if (M_EGATE.RecordFound == true)
                        {
                            // Check if reversal 
                            // Check if done before with Mask 01
                            if (M_EGATE.Matched == false & M_EGATE.LoadedAtRMCycle < WRMCycle)
                            {
                                // Previous day we had the record as 01
                                WWMask = "P1";
                                WCustomerID = M_EGATE.CustomerID; // Get the customer id too
                                WReference_TPF = M_EGATE.Reference_TPF;
                                M_EGATE.MatchMask = "1N"; // we update the old record as N1 ... came as Next day 
                                M_EGATE.Comments = "Matching Record Came next cycle";
                                M_EGATE.UpdateRecordAsUnmatchedBySeqNumber_MOBILE(SourceTable_A, M_EGATE.SeqNo);
                            }
                            if (M_EGATE.IsReversal == true & M_EGATE.LoadedAtRMCycle == WRMCycle)
                            {
                                IsReversal = true;
                        ...
                        M_EGATE.ReadTransSpecificFromSpecificTable_MOBILE(SelectionCriteria, SourceTable_B, 1, W_Application);
                        ...
                        M_EGATE.MatchMask = WWMask; // it is 01 or P1
                        M_EGATE.MatchingAtRMCycle = WRMCycle;

                        M_EGATE.ActionByUser = false;
                        M_EGATE.UserId = "";
                        M_EGATE.Authoriser = "";
                        M_EGATE.AuthoriserDtTm = NullPastDate;
                        M_EGATE.ActionType = "00";

                        if (WWMask == "P1")
                        {
                            M_EGATE.Comments = "Record exist previous cycle as not matched as Mask 10 ";
                        }
                        else
                        {
                        ...
                        M_EGATE.SettledRecord = false;
                        //Mmob.Net_TransDate = (DateTime)rdr["Net_TransDate"];
                        //Mmob.Minutes_Date = (int)rdr["Minutes_Date"];
                       
                        M_EGATE.SET_DATE = WCut_Off_Date;
                        
                        
                        if (WCustomerID != "")
                        {
                            M_EGATE.CustomerID = WCustomerID; // 
                        }
                        else
                        {
                            M_EGATE.CustomerID = "Not Available"; 
                        }

                        if (WReference_TPF != "")
                        {
                            M_EGATE.Reference_TPF = WReference_TPF;
                        }
                        else
                        {
                            M_EGATE.Reference_TPF = "Not Available";
                        }

                        
                        int NewSeqNo = 0; 
                        NewSeqNo = M_EGATE.InsertNewRecordInTable_MOBILE_MASTER(SourceTable_A);
                        if ( NewSeqNo == 0)
                        {
                        ...
                        M_EGATE.UpdateRecordAsUnmatchedBySeqNumber_MOBILE(TableId, SeqNoInTwo);
        ...
        private void UpdateDuplicatesInMasterTableForMOBILE()
        {
            ...
            M_EGATE.RecordFound = false;

            if (SeqNoInOne > 0)
            {
                TableId = SourceTable_A;
                M_EGATE.ReadTransSpecificFromSpecificTable_By_SeqNo(TableId, SeqNoInOne, 1);
            }

            if (SeqNoInOne == 0 & SeqNoInTwo > 0)
            {
                TableId = SourceTable_B;
                M_EGATE.ReadTransSpecificFromSpecificTable_By_SeqNo(TableId, SeqNoInTwo, 1);
                ...
                {
                    // 
                    WSelectionCriteria = " WHERE RRNumber ='" + M_EGATE.RRNumber + "' "
                    //+ " AND TransAmount =" + Mmob.TransAmount
                    + " AND  MatchingCateg ='" + M_EGATE.MatchingCateg + "' ";
                }
                else
                {
                    WSelectionCriteria = " WHERE RRNumber ='" + M_EGATE.RRNumber + "' "
                ...
                M_EGATE.ReadTransSpecificFromSpecificTable_By_SelectionCriteria(TableId, WSelectionCriteria, 1);

                if (M_EGATE.RecordFound == true)
                {
                    SeqNoInOne = M_EGATE.SeqNo;
                    //TableId = SourceTable_A;
            ...
            char[] WMask = M_EGATE.MatchMask.ToCharArray();
            ...
            M_EGATE.UpdateRecordAsUnmatchedByFilter_MOBILE(TableId, WSelectionCriteria);
