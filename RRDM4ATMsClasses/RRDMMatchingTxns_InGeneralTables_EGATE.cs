...
namespace RRDM4ATMs
{
    public class RRDMMatchingTxns_InGeneralTables_EGATE : Logger
    {
        public RRDMMatchingTxns_InGeneralTables_EGATE() : base() { }
        ...
        //readonly string ATMSconnectionString = ConfigurationManager.ConnectionStrings["ReconConnectionString"].ConnectionString;
...
+ "  SELECT * FROM MergedTbl ";
        ...
        public void Read_EWALLET_TXNS_MASTER_TOTALS(string W_Application, string InSelectionCriteria, int In_DB_Mode)
        {
            RecordFound = false;
            ErrorFound = false;
            ErrorOutput = "";
            ...
            if (In_DB_Mode == 2)
            {
                SqlString =
                 ...
                 + " ( "
                 + " SELECT TransAmount  "
                 + " FROM " + W_Application + ".[dbo]." + "TXNS_MASTER"
              + InSelectionCriteria
                + " UNION ALL  "
                 + " SELECT TransAmount "
                 + " FROM " + W_Application + "_MATCHED_TXNS.[dbo]." + "TXNS_MASTER"
                + InSelectionCriteria
                 + " ) "
                                         ...
                                         string InSortCriteria, int In_DB_Mode, bool InFirstCycle, DateTime InSettlementDate, int InFunction, string W_Application)
        {
            ...
            string connectionString = "";
            string MasterTable = "";
            string MasterTable_Matched = ""; 
            if (W_Application == "EGATE")
            {
                connectionString = ATMSconnectionString;
                MasterTable = W_Application + ".[dbo].[TXNS_MASTER]";
                MasterTable_Matched = W_Application+"_MATCHED_TXNS.[dbo].[TXNS_MASTER]"; 
            }
                ...
                try
                {
                        ...
                        new SqlCommand(SqlString, conn))
                    {
                        ...
                        while (rdr.Read())
                        {
                            ...
                            if (MatchMask == "10" & W_Application != "EGATE")
                            {
                                       ...
                                       string InSortCriteria, int In_DB_Mode, bool InFirstCycle, DateTime InSettlementDate, int InFunction, string W_Application)
        {
            ...
            string SourceTable_B = "EGATE_6_CLEARING_REPORT_TXNS";
                        ...
                        while (rdr.Read())
                        {
                          ...
                          //  RowSelected["Descr"] = TransType;
                           ...
                           // ReadTransSpecificFromSpecificTable_MOBILE_ETI375
                            M_EGATE.ReadTransSpecificFromSpecificTable_MOBILE_ETI375(SelectionCriteria375, SourceTable_B, 2, W_Application);
                            if (M_EGATE.RecordFound)
                            {
                                //Is_DiffAmt_ETI375 = true;
                                SecondAmt = M_EGATE.Amount_Merchant;
                                DifAmt_EGA375 = TransAmount - SecondAmt;
                            }

                            RowSelected["Amount_Merchant"] = SecondAmt;
                            RowSelected["Amount_Difference"] = DifAmt_EGA375;
    ...
    new SqlConnection(connectionString))
        ...
        public void ReadTablebyCycleAndCategory_EGA375(string InMatchingCateg, string In_Merchant_ID, int RM_Cycle, string W_Application)
        {
            RecordFound = false;
            ErrorFound = false;
            ErrorOutput = "";
            ...
            if (InMatchingCateg == "EGA375" & In_Merchant_ID =="")
            {
                ...
                FileA = Mcs.SourceFileNameA; // EG TPF_TXNS
                FileA = "TXNS_MASTER"; // FOR EGATE
                FileB = Mcs.SourceFileNameB; // EG TPF_TXNS 


                FileA_RRDM = W_Application + ".[dbo]." + "TXNS_MASTER";
                FileA_RRDM_MATCHED = W_Application + "_MATCHED_TXNS.[dbo]." + "TXNS_MASTER";
            ...
            //
            FileA = "[EGATE].[dbo].[BULK_EGATE_1_SALARY_WITHDRAWLS_TXNS_ALL]"; // SALARIES
            FileB = "[EGATE].[dbo].[BULK_EGATE_2_AGENTS_PAY_TXNS_ALL]"; // AGENTS
            FileC = "[EGATE].[dbo].[EGATE_1_SALARY_WITHDRAWLS_TXNS_DUPLICATES]"; // Duplicates for Salaries 
...
+ "  SELECT * FROM MergedTbl ";
       ...
       // public string SenderTelephone;
        ...
        public void ReadTransSpecificFromSpecificTable_MOBILE_ETI375(string InSelectionCriteria, string InTableName, int In_DB_Mode, string W_Application)
        {
            ...
            //[EGATE].[dbo].[EGATE_2_AGENTS_PAY_TXNS]
            ...
            if (In_DB_Mode == 2)
            {
                ...
                SqlString =
                    ...
                    + " ( "
                    + " SELECT *  "
                    + " FROM [EGATE].[dbo].[EGATE_2_AGENTS_PAY_TXNS] "
                 + InSelectionCriteria
                   + " UNION ALL  "
                    + " SELECT * "
                    + " FROM [EGATE_MATCHED_TXNS].[dbo].[EGATE_2_AGENTS_PAY_TXNS] "
                   + InSelectionCriteria
                    + " ) "
        ...
        public bool FindRecordsFromFile_X(string InOperator, string InFileID, string InMatchingCateg,
                                                                                 ...
                                                                                 , int InMode, int In_DB_Mode)
        {
            ...
            try
            {
                ...
                string PhysicalName = "[EGATE].[dbo]." + "[" + Msf.InportTableName + "]";
                ...
                if (In_DB_Mode == 2)
                {
                    if (InMode == 1) // This fills the Table by Trace, Terminal and Date (includes Succesfull and not) 
                    {
                        ...
                        //{
                           ...
                           + " ( "
                           + " SELECT *  "
                           + " FROM [EGATE].[dbo]." + Msf.InportTableName
                           + " WHERE "
                           + " Cast(TransDate as Date) = @Net_TransDate "
                             + " AND MatchingCateg = @MatchingCateg "
                           //+ " AND TerminalId = @TerminalId "
                           + " AND "
                           + " (RRNumber =@RRNumber ) "
                           ...
                           + " SELECT * "
                           + " FROM[EGATE].[dbo]." + Msf.InportTableName
                           + " WHERE Cast(TransDate as Date) = @Net_TransDate "
                             + " AND MatchingCateg = @MatchingCateg "
                           //+ " AND TerminalId = @TerminalId "
                           + " AND (RRNumber =@RRNumber ) "
                           + " AND TransAmount =@TransAmt "
                ...
                if (DataTableAllFields.Rows.Count > 0)
                {
                    ReturnRecordFoundInUniversal = true;
                }
                else
...
+ "  ";
      ...
      //,[MatchingAtRMCycle]
                    ...
                    // Set it as Matched to be moved to Matched TXNs
                    // Matched = 1 , MatchMask = '11'
                    conn.Open();
                    using (SqlCommand cmd =
                        new SqlCommand("UPDATE " + InTableName
                              + " SET IsMatchingDone = 1 ,Matched = 1 , MatchMask = '11' ,MatchingAtRMCycle = @MatchingAtRMCycle , Comments = 'Records Moved to Master' "
