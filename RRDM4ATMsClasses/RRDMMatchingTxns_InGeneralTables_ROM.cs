...
namespace RRDM4ATMs
{
    public class RRDMMatchingTxns_InGeneralTables_ROM : Logger
    {
        public RRDMMatchingTxns_InGeneralTables_ROM() : base() { }
       ...
       + " WHERE SeqNo=@SeqNo "
        ...
        public bool FindRecordsFromMasterRecord_ROM(
           string InTerminalId, string InRRNumber, string InUTRNNO,string InFileID,  int InMode, int In_DB_Mode)
        {
            RecordFound = false;
            ErrorFound = false;
            ErrorOutput = "";
            ...
            try
            {
                ...
                if (In_DB_Mode == 2)
                {
                    ...
                    if (InMode == 5) // This fills the Table by Trace, Terminal and Date (includes Succesfull and not) 
                    {

                        TableDetails_RAW = new DataTable();
                        TableDetails_RAW.Clear();
                        if (InUTRNNO != "" || InFileID == "COREBANKING" || InFileID == "Switch_IST_Txns_TWIN")
                        {
                 ...
                 new SqlConnection(ATMSconnectionString))
                    try
                    {
                        ...
                        using (SqlDataAdapter sqlAdapt = new SqlDataAdapter(SqlString, conn))
                        {
                           ...
                           // sqlAdapt.SelectCommand.Parameters.AddWithValue("@Net_TransDate", InTransDate.Date);
                            sqlAdapt.SelectCommand.Parameters.AddWithValue("@TerminalId", InTerminalId);
                            sqlAdapt.SelectCommand.Parameters.AddWithValue("@UTRNNO", InUTRNNO);
...
+ " Order By FullTraceNo ";
        ...
        public void UpdateReversalsForMask_By_Trace(string InTable, string InMatchingCateg, string InTerminalId,
                                        DateTime InTransDate,
                                        string UTRNNO, decimal InTransAmt, string InCardNumber, string InMask)
        {
            int count = 0;
            string SQLCmd = "UPDATE " + InTable
                   + " SET Mask = @Mask "
...
+ "  ";
        ...
        public DateTime ReadAndFindDateBasedOnDetails_2_ROM(string InTable, string InMatchingCateg, string InTerminalId,
                                        DateTime InTransDate,
                                        string InUTRNNO, decimal InTransAmt, string InCardNumber)
        {
            RecordFound = false;
            ErrorFound = false;
            ErrorOutput = "";
       ...
       + " ";
        ...
        public DateTime ReadAndFindMaxDtForFile_MOBILE(string InApplication, string InTable, int InRMCycle, string InOriginFileName)
        {
            ...
            string ETISALATConnectionString = ConfigurationManager.ConnectionStrings
                  ["ETISALATConnectionString"].ConnectionString;
