...
namespace RRDM4ATMs
{
    public class RRDMMatchingTxns_MasterPoolATMs : Logger
    {
        public RRDMMatchingTxns_MasterPoolATMs() : base() { }
        ...
        readonly string connectionStringITMX = AppConfig.GetConnectionString("ReconConnectionString");
        ...
        public void ReadTablePoolDataToGetMaskTableMasks_1_E_Wallet(int InRMCycleNo, string W_Application)
        {
            ...
            {
                MasterTable = W_Application + ".[dbo]." + W_Application + "_TPF_TXNS_MASTER"; 
            }
     ...
     //   _E_Wallet(int InRMCycleNo, string W_Application)
        ...
        public void ReadTablePoolDataToGetMaskTableMasks_2_HST_E_Wallet(int InRMCycleNo, string W_Application)
        {
            ...
            string MasterTable_HST = "";

            if (W_Application == "ETISALAT" || W_Application == "QAHERA")
            {
                MasterTable_HST = W_Application + "_HST.[dbo]." + W_Application + "_TPF_TXNS_MASTER";
            }
            if (W_Application == "EGATE")
            {
                MasterTable_HST = W_Application+ "_HST.[dbo]."+ "TXNS_MASTER";
            }
...
+ "  SELECT Count(*) As TotalCounted FROM MergedTbl ";
        ...
        public DateTime ReadAndFindMaxDateTimeForMpa_NonATM_2(string InFile, string InMatchingCateg, int In_DB_Mode)
        {
           ...
           + " WHERE  "
                       + "    ([MatchingCateg] = @MatchingCateg)  "
                       + " AND ([IsMatchingDone] = 0) AND [TXNDEST] IN ('123','124' ,'1', '4') "
                       + " AND (ResponseCode = '0') ";
            }
       ...
       // public int Count;
        public void DeleteDuplicates_tblHstAtmTxnsROM(int InLoadedAtRMCycle)
        {
            ErrorFound = false;
            ErrorOutput = "";
            Count = 0;
      ...
      //,[AUTHNUM]
        string CMD_Details = " WITH cte AS( "
                      ...
                      + " WHERE TranDesc = 'DEPOSIT'  "
                      + " ) "
                      //+ " DELETE FROM cte "
                      + " DELETE FROM cte  where (row_num = 2 and Result = 'ERROR') ";
            //string SqlCommand = "DELETE FROM [RRDM_Reconciliation_ITMX].[dbo].[tblMatchingTxnsMasterPoolATMs] "
            //                + " WHERE MatchingCateg = @MatchingCateg "; 
        ...
        // public int Count;
        public void DeleteDuplicates_tblHstAtmTxnsROM_EURO(int InLoadedAtRMCycle)
        {
            ErrorFound = false;
            ErrorOutput = "";
            Count = 0;
            ...
            string CMD_Details = " WITH cte AS( "
                          ...
                          + " ) "
                          //+ " DELETE FROM cte "
                          + " Select * From cte where (row_num = 2 and Result = 'OK') "; 
                    ...
                    try
                    {
                        ...
                        using (SqlCommand cmd =
                            new SqlCommand("UPDATE [ATM_MT_Journals_AUDI].[dbo].[tblHstAtmTxnsROM] "
                                  + " SET AUTHNUM = '9999'    "
                           + " WHERE UTRNNO = '250401224018850721'"
                         + "  OR UTRNNO = '250401224018897881' "
                          + " OR UTRNNO = '250401224018869381' "
                          //-or UTRNNO = '250401224018850721'
                          + " or UTRNNO = '250403224364758701' "
                            + "or UTRNNO = '250403224364740661'"
                            + "or UTRNNO = '250404224664080461'"
                            + "or UTRNNO = '250404224626258921'"
                            + "or UTRNNO = '250403224503839871'"
                            + "or UTRNNO = '250403224503825791'"
                            , conn))
                        {
