...
namespace RRDM4ATMs
{
    public class RRDM_LoadFiles_InGeneral_EMR_ABE : Logger
    {
        public RRDM_LoadFiles_InGeneral_EMR_ABE() : base() { }
        //
                                           ...
                                           , string InFullPath, string InCondition, string InFullPath2, int InFlogSeqNo)
        {
            ...
            // ABE in data base but ABEN in 123 file 
            ...
            switch (InTableA)
            {
                ...
                case "MEEZA_GLOBAL_GTW":
                    {

                        stpErrorText = DateTime.Now + "_" + "01_Start_Loading_MEEZA_GLOBAL_GTW" + "\r\n";

                        Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);

                        InsertRecords_GTL_MEEZA_GLOBAL_GTW(InTableA, InFullPath, WReconcCycleNo);
                ...
                case "MEEZA_GLOBAL_INT":
                    {

                        stpErrorText = DateTime.Now + "_" + "01_Start_Loading_MEEZA_GLOBAL_INT" + "\r\n";

                        Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);

                        InsertRecords_GTL_MEEZA_GLOBAL_INT(InTableA, InFullPath, WReconcCycleNo);
                ...
                case "VISA_CARD_ABE":
                    {
                        ...
                        InsertRecords_GTL_VISA_CARD_ABE(InTableA, InFullPath, WReconcCycleNo);
                ...
                // InsertRecords_GTL_HOST_TO_HOST_124
                case "HOST_TO_HOST_124":
                    {

                        stpErrorText = DateTime.Now + "_" + "01_Start_Loading_HOST_TO_HOST_124" + "\r\n";

                        Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);

                        InsertRecords_GTL_HOST_TO_HOST_124(InTableA, InFullPath, WReconcCycleNo);
                ...
                // InsertRecords_GTL_HOST_TO_HOST_EBC
                case "HOST_TO_HOST_EBC":
                    {

                        stpErrorText = DateTime.Now + "_" + "01_Start_Loading_HOST_TO_HOST_EBC" + "\r\n";

                        Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);

                        InsertRecords_GTL_HOST_TO_HOST_EBC(InTableA, InFullPath, WReconcCycleNo);
                          ...
                          , int InReconcCycleNo)
        {
            ...
            // Reassign File name
            InOriginFileName = InOriginFileName;
            ...
            //
            // Correct CARDPRODUCT due to bad characters 
            // 
            // 

            SQLCmd = " Update [RRDM_Reconciliation_ITMX].[dbo].BULK_" + InOriginFileName
                     + " SET [CARDPRODUCT] = LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(REPLACE([CARDPRODUCT], CHAR(10), "
            ...
            SQLCmd = " INSERT INTO [RRDM_Reconciliation_ITMX].[dbo].BULK_Switch_IST_Txns_ALL " //  We insert in the second IST
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    conn.StatisticsEnabled = false;
                    conn.Close();
                    stpErrorText = stpErrorText + "Cancel During Creation of BULK_Switch_IST_Txns_ALL";
                    stpReturnCode = -1;
            ...
            SQLCmd = "INSERT INTO [RRDM_Reconciliation_ITMX].[dbo].[Switch_IST_Txns]"
                + "( "
                 ...
                 + ",[FullTraceNo] "
                 + ",[AUTHNUM]"
                 + ",[Comment]" // Use a temporary field to host VISAI and VISAD 
                 ...
                 + " ,@Origin "
                   + " ,case "
                   //+ " WHEN (TXNSRC = '1' AND TXNDEST = '1' AND Left(MASK_PAN,6) = '489322')  THEN '" + PRX + "201' " // Only DEBIT CARD for Flex 
                   // + " WHEN (TXNSRC = '1' AND TXNDEST = '1') AND Left(MASK_PAN,6) <> '489322'  THEN '" + PRX + "203'" // Prepaid OR Meeza
                   + " WHEN (TXNSRC = '1' AND TXNDEST = '1' )  THEN '" + PRX + "201' " // Only DEBIT CARD two types , one debit and the other meeza
                    + " WHEN (TXNSRC = '1' AND TXNDEST = '4') THEN '" + PRX + "207' " // All outgoing Visa
                                                                                      //+ " WHEN (TXNSRC = '1' AND TXNDEST = '5')  THEN '" + PRX + "206' " // All outgoing Master
                   + " WHEN (TXNSRC = '1' AND TXNDEST = '123') THEN '" + PRX + "208' " // All outgoing 123 going Meeza
                   + " WHEN (TXNSRC = '1' AND TXNDEST = '124') THEN '" + PRX + "209' " // All outgoing 124 


                  // AND left([TXN], 1) <> '0' 
                  + " WHEN (TXNSRC = '123' AND TXNDEST= '1'  AND Left(MASK_PAN,6 ) = '489322' AND left([TXN], 1) <> '0'  ) "
                                                        + " THEN '" + PRX + "277' " // All incoming 123 NET - DEBIT
                                                                                    // Maybe we have this too '0-POS Purchase' if REf do not agree
                   + " WHEN (TXNSRC = '123' AND TXNDEST= '1' AND Left(MASK_PAN,6) <> '489322' AND left([TXN], 1) <> '0' ) " // '0-POS Purchase'
                                                        + " THEN '" + PRX + "278' " // All incoming 123 NET - NON DEBIT

                  // + " WHEN (TXNSRC = '124' AND TXNDEST = '1') THEN '" + PRX + "286' " // All incoming 124 - There are no Incoming
                 ...
                 + " , CAST(ISNULL(try_convert(datetime, [LOCAL_DATE], 103 ), '1900-01-01') AS DateTime)"
                    ...
                    + "+ CAST(substring(right('000000' + [LOCAL_TIME], 6), 1, 2) + ':' " // hh
                    + "+ substring(right('000000' + [LOCAL_TIME], 6), 3, 2)" // mm
                    + " + ':' + substring(right('000000' + [LOCAL_TIME], 6), 5, 2) as datetime)" // ss
                 + ",case "
                ...
                // RIGHT('000000000000' + TRIM(REFERENCENUMBER), 12)
                 + " , ISNULL(RIGHT('000000000000' + TRIM(REFNUM), 12), '') " // RRN

                 + ",[TRACE] "
                  + ",ISNULL(AUTHNUM, '')" // Auth Number 
               
                 + " ,CASE" // VISA INTERNATIONAL AND LOCAL IN COMMENT 
                 + " WHEN ([TXNSRC] = '1' AND [TXNDEST] = '123' AND CARDPRODUCT = 'VISAI') THEN 'VISAI'  "
                 + " WHEN ([TXNSRC] = '1' AND [TXNDEST] = '123' AND CARDPRODUCT = 'VISAD') THEN 'VISAD'  "
                 + " WHEN ([TXNSRC] = '1' AND [TXNDEST] = '123' AND left(MASK_PAN, 6) = '494608') THEN 'VISAD'  "
                 + " ELSE '' " // 
                 + " END "
                 ...
                 + " WHERE  LOCAL_DATE <> '01-JAN-80' AND TXN <> '200000-Refund' AND "
           + " ( "
           + " (left([TXN], 2) IN "
           + "( '10'," // Withdrawl 
           + "  '11'," // Withdrawl 
            ...
            if (IsABE_TWIN == true)
            {
                ...
                + "( "
                 ...
                 + " ,[Origin] "
                  ...
                  //                                                                  //                                                                                                    //  + " WHEN (TXNSRC = '1' AND TXNDEST = '5') AND left([TransDescr], 2) <> '81' THEN '"+PRX+"235' " // All outgoing Master
                  //       + " WHEN ([TXNSRC] = '1' AND [TXNDEST] = '123' AND CARDPRODUCT = 'VISA') THEN 'VISA'  "                                                            // + " WHEN (TXNSRC = '1' AND TXNDEST = '5' AND left([TransDescr], 2) = '81' AND TransAmt<> '0') THEN '"+PRX+"251' "
                  + " WHEN (TXNSRC = '1' AND TXNDEST = '123' AND Comment = '' ) THEN '" + PRX + "279' " // All outgoing 123 
                 // + " WHEN (TXNSRC = '1' AND TXNDEST = '123' AND Comment = 'VISAI' ) THEN '" + PRX + "281' " // This is Visa
                    + " WHEN (TXNSRC = '1' AND TXNDEST = '123' AND Comment = 'VISAI' ) THEN '" + PRX + "279' " // This is Visa
                  + " WHEN (TXNSRC = '1' AND TXNDEST = '123' AND Comment = 'VISAD' ) THEN '" + PRX + "282' " // This is Visa
                  + " WHEN (TXNSRC = '1' AND TXNDEST = '124' ) THEN '" + PRX + "285' "
      ...
      //     + " end "
          ...
          + ", [SET_DATE] "
                 ...
                 + " WHERE  SeqNo > @SeqNo  AND  "
                   + " ( (TXNSRC = '1' AND TXNDEST = '123') OR (TXNSRC = '1' AND TXNDEST = '124') ) " // 
                   + " "  // 

                     ;
                //  + " WHEN ([TXNSRC] = '1' AND [TXNDEST] = '123' AND CARDPRODUCT = 'VISA') THEN 'VISA'  "
                using (SqlConnection conn = new SqlConnection(recconConnString))
                    try
                    {
                        conn.StatisticsEnabled = true;
            ...
            //
            // We insert in TWIN a second record for Visa
            // TO BE MATCHED wiTH VISA REPORT
            // 
            bool Visa225 = false; 
            ...
            if (Visa225 == true)
            {
                ...
                + "( "
                 ...
                 + " ,[Origin] "
                    // + " ,[MatchingCateg] "
                    + " ,CASE"
                    + " WHEN Comment = 'VISA' THEN '" + PRX + "225' " // All outgoing Visa                                                                    
                   + " else 'Not_Def' "
                    + " END "
      ...
      + " ,[TerminalType] "
       ...
       + " ,case"
           + " WHEN Comment = 'VISA' THEN [EXTERNAL_DATE] " // All outgoing Visa       
           + " else [TransDate] "
           + " end "
      + " ,[TransType] "
      + " ,[TransDescr] "
      ...
      + " ,4 " // 4 For VISA
      + " ,case"
          + " WHEN Comment='VISA' THEN [TransDate] " // All outgoing Visa       
          + " else [EXTERNAL_DATE] "
         + " end "
          ...
          + ", [SET_DATE] "
                 ...
                 + " WHERE  SeqNo > @SeqNo  AND Comment='VISAI' "
                      ;
                ...
                using (SqlConnection conn = new SqlConnection(recconConnString))
                    ...
                    catch (Exception ex)
                    {
                        conn.StatisticsEnabled = false;
                        conn.Close();
                        stpErrorText = stpErrorText + "Cancel During Second _IST_TWIN";
                ...
                //**********************************************************
                stpErrorText += DateTime.Now + "_" + "Stage_04.2_IST_second TWIN_Created..Records:.." + Counter.ToString() + "\r\n";
                stpErrorText += DateTime.Now + "_" + "Time Taken in Ms " + commandExecutionTimeInMs.ToString() + "\r\n";
                Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);
            ...
            SQLCmd = "UPDATE [RRDM_Reconciliation_ITMX].[dbo].[Switch_IST_Txns] "
             ...
             + " ,[ProcessedAtRMCycle] = @ProcessedAtRMCycle"
               + " ,[Comment] = 'Presenter' "
            + " WHERE   Processed = 0 AND LEFT(FullTraceNo,1) in ( '2','4' ) AND ResponseCode = '112' "
                                           ...
                                           , int InReconcCycleNo)
        {
            ...
            SQLCmd =
                ...
                + ",[AccNo] "
                 ...
                 + "@OriginFile"
                 + " ,@Origin"
              //  + " ,'" + PRX + "299'"  // SET MATCHING CATEGORY to 299 
                 ...
                 + " WHEN LEFT(Terminalid, 2) = 'AB' AND Left(CardNumber,6) = '489322' THEN 'EMR201' "
                 + " WHEN LEFT(Terminalid, 2) = 'AB' AND Left(CardNumber,6) <> '489322' THEN 'EMR203' "
                 + " WHEN LEFT(Terminalid, 2) <> 'AB' THEN 'EMR277' "
                 + " End "

                 + ", ISNULL([TERMINALID], '') " // Terminal 
                 + ", ISNULL(CardNumber,'') " // CARD Number 
                ...
                + " WHEN [TRANSACTIONCODE] = '21' AND [REVERSAL] = '0' THEN '11'  " //  Correction for cases 507803 BIN
                + " WHEN [TRANSACTIONCODE] = '21' AND [REVERSAL] = '1' THEN '21'  " //  Reversal for cases 507803 BIN
                + " WHEN [TRANSACTIONCODE] = '23' AND [REVERSAL] = '0' THEN '23'  " // Deposit
                + " WHEN [TRANSACTIONCODE] = '23' AND [REVERSAL] = '1' THEN '13'  " // Reversal for Deposit 
                ...
                + " WHEN [TRANSACTIONCODE] = '21' AND [REVERSAL] = '0' THEN 'TXN 507803 type'  " // TXN 507803 type 
                + " WHEN [TRANSACTIONCODE] = '21' AND [REVERSAL] = '1' THEN 'Reversal 507803 type'  " // TXN 507803 type 
                  + " else 'Not Defined' "
                  + "end "
                ...
                // + ",ISNULL([REFERENCENUMBER] , '') " // in place RRN 
                  + " , ISNULL(RIGHT('000000000000' + TRIM(REFERENCENUMBER), 12), '') " // RRN
                 + ",ISNULL([AUTHORIZATIONCODE] , '')" // We put Authorisation code here                                                                  //   
                                                       //  + ", ISNULL([PAN_MASK], '') "  // 
                                           ...
                                           , int InReconcCycleNo)
        {
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During MEEZA_LCL BULK Truncate";
                    stpReturnCode = -1;
            ...
            SQLCmd = "INSERT INTO [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_LCL]  "
                + "( "
                 ...
                 + " ,@Origin"
                     ...
                     + ",case " // MatchingCateg
                 + " WHEN TerminalOwner= 'ABEN' THEN '" + PRX + "279'  "
                // + " WHEN (TerminalOwner= 'ABEN' AND LEft(PAN,6) ='457376') THEN '" + PRX + "281'  "
                 + " else '" + PRX + "277'  "
                 + "end "
                 ...
                 + ",RIGHT('000000000000' + RRN, 12) " // RRN insert leading zeros 
                  ...
                  + ",case " // Terminal Id
                 + " WHEN TerminalOwner= 'ABEN'  THEN left (TerminalId,8)  "
                 + " else TerminalId "
                 + "end "
                 ...
                 + ",case " // Origin 
                 + " WHEN TerminalOwner= 'ABEN' THEN '1'  " // ABE
                 + " else '8' " // Other 
                 + "end "

                 + ",case " // Destination
                 + " WHEN TerminalOwner= 'ABEN' THEN '8'  " // ABE
                 + " else '1' "
                 + "end "
                 ...
                 + " WHERE (Cast(TransactionAmount as decimal(18,2))> 0 ) "
                + " ";
            // Deposit
            using (SqlConnection conn = new SqlConnection(recconConnString))
                try
            ...
            //MAKE SET DATE One day less to agree with IST
            // WE DO NOT KNOW WHY 
            // For now we will get it from IST to be sure 


            SQLCmd = " UPDATE [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_LCL] "
                         + " SET "
                         + "   [SET_DATE] = DATEADD(day, -2 , SET_DATE )    "
                         + " WHERE LoadedAtRMCycle =  @LoadedAtRMCycle "
                         ;
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During Upadting of SET Date";
            ...
            //
            // UPDATE CATEGORY AND Date seconds with IST 
            //
          ...
          + " AND t1.TransAmt = t2.TransAmt "
          //+ " AND t1.TransDate = t2.Minutes_Date  "
          + " AND t1.AUTHNUM = t2.AUTHNUM "

          + " WHERE  "
          + "(t1.Processed = 0  ) "
          + " AND (t2.Processed = 0 OR (t2.Processed=1 AND t2.Comment = 'Reversals') ) AND  (t2.MatchingCateg = '"
          ...
          ; // For not processed yet records
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During Meeza LCL Updating For date and time";
                    CatchDetails(ex);
                    return;
                }
          ...
          + " WHERE  "
          + "(t1.Processed = 0 And t1.AmtFileBToFileC = 0 ) "
          ...
          ; // For not processed yet records
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During LCL Updating For date and time";
                    CatchDetails(ex);
                    return;
                }
            ...
            // INSERT In REVERSALS for MEEZA_LCL
            string FileId = InOriginFileName;
       ...
       ;
        ...
        //*******************
        //// CREATE MEEZA_GLOBAL GTW
        /////****************
        public void InsertRecords_GTL_MEEZA_GLOBAL_GTW(string InOriginFileName, string InFullPath
                                           , int InReconcCycleNo)
        {
            ...
            // Truncate Table MEEZA_GTW FOR BULK
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During MEEZA_GTW BULK Truncate";
                    stpReturnCode = -1;
            ...
            //       
            // CREATE MEEZA_GTW from BULK
            //

            SQLCmd = "INSERT INTO [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_GTW]  "
                + "( "
                 + " [OriginFileName] "
                 ...
                 + " ,@Origin"
                     ...
                     + ",case " // MatchingCateg
                 + " WHEN TerminalOwner= 'ABEN'   AND IsInternational = 'TRUE'  THEN '" + PRX + "281'  " // International VISA Cards
                 + " WHEN TerminalOwner= 'ABEN'   AND IsInternational = 'FALSE'  THEN '" + PRX + "282'  " // Local VISA Cards
                 + " else '" + PRX + "999'  "
                 + "end "
            ...
            stpErrorText += DateTime.Now + "_" + "Insert in MEEZA_GLOBAL_GTW with records.." + stpLineCount.ToString() + "\r\n";
            stpErrorText += DateTime.Now + "_" + "Time Taken in Ms " + commandExecutionTimeInMs.ToString() + "\r\n";
            Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);
            ...
            SQLCmd = " UPDATE [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_GTW] "
                         + " SET "
                         + "   [SET_DATE] = DATEADD(day, -2 , SET_DATE )    "
                         + " WHERE LoadedAtRMCycle =  @LoadedAtRMCycle "
                         ;
         ...
         //   SQLCmd =
         // " UPDATE [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_GTW] "
         // + " SET "
         //  + " [MatchingCateg] = t2.[MatchingCateg] " // BDC 277 or BDC278 
         //                                             //+ ", [TransType] = t2.[TransType] "
         //                                             // + " ,[TransDate] = t2.[TransDate] "
         ...
         //  + " FROM [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_GTW] t1 "
         // + " INNER JOIN [RRDM_Reconciliation_ITMX].[dbo].[Switch_IST_Txns] t2"
         ...
         //   Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);
          ...
          " UPDATE [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_GTW] "
          + " SET"
        //   + " [MatchingCateg] = t2.[MatchingCateg] " // Here is BDC281 or 282
          ...
          + " WHERE  "
          + "(t1.Processed = 0 And t1.AmtFileBToFileC = 0 ) "
          + " AND (t2.Processed = 0 OR (t2.Processed=1 AND t2.Comment = 'Reversals') ) AND  t2.MatchingCateg In ('EMR281', 'EMR282')" // '" + PRX + "281' "
          ; // For not processed yet records
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During MEEZA_GTW Updating For date and time";
                    CatchDetails(ex);
                    return;
                }

            stpErrorText += DateTime.Now + "_" + "GTW Update Date and Time from IST..." + stpLineCount.ToString() + "\r\n";
            stpErrorText += DateTime.Now + "_" + "Time Taken in Ms " + commandExecutionTimeInMs.ToString() + "\r\n";
            Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);
            ...
            // INSERT In REVERSALS for MEEZA_GTW
            string FileId = InOriginFileName;
          ...
          + ", a.TXNSRC As TXNSRC_4,b.TXNSRC as TXNSRC_2 "
          + ", a.TXNDEST As TXNDEST_4,b.TXNDEST as TXNDEST_2 "
          + " FROM [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_GTW] A "
          + " LEFT JOIN [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_GTW] B "
          + " ON A.TerminalId = B.TerminalId "
          //+ " AND A.CardNumber = B.CardNumber "
          + " AND A.TransAmt = B.TransAmt "
          + " AND A.RRNumber = B.RRNumber "
       ...
       ;
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                try
                {
                    ...
                    if (Environment.UserInteractive & TEST == true)
                    {
                        System.Windows.Forms.MessageBox.Show("REVERSAL ENTRIES Inserted for MEEZA_GLOBAL_GTW" + Environment.NewLine
                                  + "..:.." + Counter.ToString());
                    }
                ...
                catch (Exception ex)
                {
                    conn.Close();
                    ErrorFound = true;
                    stpErrorText = stpErrorText + "Cancel At _Reversals_MEEZA_GLOBAL_GTW";
                    stpReturnCode = -1;
            ...
            Counter = 0;
            //FileId = "Flexcube";
            string TableId = "[RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_GTW]";
            int SeqNo_2;
            int SeqNo_4;
            ...
            stpErrorText += DateTime.Now + "_" + "MEEZA_GLOBAL_GTW Finishes with SUCCESS.." + "\r\n";
        ...
        //// CREATE MEEZA_GLOBAL GTW
        /////****************
        public void InsertRecords_GTL_MEEZA_GLOBAL_INT(string InOriginFileName, string InFullPath
                                           , int InReconcCycleNo)
        {
            ...
            // Truncate Table MEEZA_INT FOR BULK
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During MEEZA_INT BULK Truncate";
                    stpReturnCode = -1;
            ...
            //       
            // CREATE MEEZA_INT from BULK
            // WE INSERT IN LCL INSTEAD OF INT
            //
            SQLCmd = "INSERT INTO [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_LCL]  "
           // SQLCmd = "INSERT INTO [RRDM_Reconciliation_ITMX].[dbo].[MEEZA_GLOBAL_INT]  "
                + "( "
                 + " [OriginFileName] "
                 ...
                 + " ,@Origin"
                     ...
                     + ",case " // MatchingCateg
                 + " WHEN TerminalOwner= 'ABEN' THEN '" + PRX + "279'  " // Not 282
                 + " else '" + PRX + "999'  "
                 + "end "
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                try
                {
                    ...
                    using (SqlCommand cmd =
                        new SqlCommand(SQLCmd, conn))
                    {
                        cmd.Parameters.AddWithValue("@OriginFile", WFileSeqNo+"Meeza_INT");
                        cmd.Parameters.AddWithValue("@Origin", Origin);
                        cmd.Parameters.AddWithValue("@TerminalType", WTerminalType);
                        cmd.Parameters.AddWithValue("@Operator", WOperator);
                        cmd.Parameters.AddWithValue("@LoadedAtRMCycle", InReconcCycleNo);
            ...
            stpErrorText += DateTime.Now + "_" + "Insert in MEEZA_GLOBAL_INT with records.." + stpLineCount.ToString() + "\r\n";
            stpErrorText += DateTime.Now + "_" + "Time Taken in Ms " + commandExecutionTimeInMs.ToString() + "\r\n";
            Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);
          ...
          ; // For not processed yet records
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During MEEZA_INT Updating For date and time";
                    CatchDetails(ex);
                    return;
                }

            stpErrorText += DateTime.Now + "_" + "INT Update Date and Time from IST..." + stpLineCount.ToString() + "\r\n";
            stpErrorText += DateTime.Now + "_" + "Time Taken in Ms " + commandExecutionTimeInMs.ToString() + "\r\n";
            Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);
            ...
            // INSERT In REVERSALS for MEEZA_INT
            string FileId = InOriginFileName;
       ...
       ;
        ...
        // VISA_CARD_ABE        //
        // GTL = GET TRANSFORM LOAD
        public void InsertRecords_GTL_VISA_CARD_ABE(string InOriginFileName, string InFullPath
                          , int InReconcCycleNo)
        {
      ...
      + " ,case"
        ...
        //*******************
        //// CREATE HOST_TO_HOST_124
        /////****************
        public void InsertRecords_GTL_HOST_TO_HOST_124(string InOriginFileName, string InFullPath
                                           , int InReconcCycleNo)
        {
            ...
            // Truncate Table HOST_TO_HOST_124 FOR BULK
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During HOST_TO_HOST_124 BULK Truncate";
                    stpReturnCode = -1;
            ...
            SQLCmd = "INSERT INTO [RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_124]  "
                + "( "
                 + " [OriginFileName] "
                 ...
                 + " ,@Origin"
                     ...
                     + ",case " // MatchingCateg
                 + " WHEN Left(Terminal_Id, 2) = 'AB' THEN '" + PRX + "285'  "
                 + " WHEN Left(Terminal_Id, 2) <> 'AB' THEN '" + PRX + "286'  "
                 + " else '" + PRX + "299'  "
                 + "end "

                 + ", @TerminalType"
                 + " , Left(LocalDateTime, 19) "
                 // + ", CAST([TransactionDate] As datetime) " // Transaction Date without minutes 
                 //+ " , CAST('20' + SUBSTRING(LocalDateTime, 7, 2)+ SUBSTRING(LocalDateTime, 4, 2) + SUBSTRING(LocalDateTime, 1, 2) + ' ' + SUBSTRING(LocalDateTime, 10, 5) AS datetime) " // Trans Date
                 + ",CASE " // Transaction Type
                 + " when (TransactionCode = '17' AND Action_Code = '1') THEN 11  "
                 + " when (TransactionCode = '18' AND Action_Code = '90') THEN 21  "
                 + " when (TransactionCode = '21' AND Action_Code = '21') THEN 23  "
                 + " when (TransactionCode = '22' AND Action_Code = '90') THEN 13  "
                 + " else 11 "
                 + " END "

                 + ",case " // DESCRIPTION
                 + " when (TransactionCode = '17' AND Action_Code = '1') THEN 'Debit Txn'  "
                 + " when (TransactionCode = '18' AND Action_Code = '90') THEN 'Reversal of Debit'  "
                 + " when (TransactionCode = '21' AND Action_Code = '21') THEN 'Deposit Txn_From_Master'  "
                 + " when (TransactionCode = '22' AND Action_Code = '90') THEN 'Reversal of Credit'  "
                 + " else 'NOT DEFINED_EMR286'  "
                 + "end "
                 + ", '818' " // Ccy
                  + ",case " // Transaction_Amount
                 + " when (TransactionCode = '17' AND Action_Code = '1') THEN CAST(Transaction_Amount As decimal(18,2))  "
                 + " when (TransactionCode = '18' AND Action_Code = '90') THEN -CAST(Transaction_Amount As decimal(18,2))  " // Make it Possitive
                 + " when (TransactionCode = '21' AND Action_Code = '21') THEN CAST(Transaction_Amount As decimal(18,2))  "
                 + " else CAST(Transaction_Amount As decimal(18,2))  "
                 + "end "

                 + ", 0 " // TRace ..SystemTraceAuditNumber 
                 + ",RIGHT('000000000000' + RRN, 12) " // RRN insert leading zeros 

                  + ",ISNULL(RIGHT('000000' + AUTH_ID, 6), '' ) " // AuthNum
                                                                  //  + ",ISNULL(AuthNum, '' ) " // AuthNum
                  + ",case " // Terminal Id
                 + " WHEN Left(Terminal_Id, 2) = 'AB'  THEN left (Terminal_Id,8)  "
                 + " else Terminal_Id "
                 + "end "
                 //  + ",  right (TerminalId,8) " // Terminal Id
                // + ", ISNULL(left([PAN], 6) + '******' + right([PAN], 4), '') "  // CARD  
                 ...
                 + ",case " // Origin 
                 + " WHEN Left(Terminal_Id, 2) = 'AB' THEN '1'  " // ABE
                 + " else '124' " // Other 
                 + "end "

                 + ",case " // Destination
                 + " WHEN Left(Terminal_Id, 2) = 'AB' THEN '124'  " // ABE
                 + " else '1' "
                 + "end "
                 + ", cast(Left(LocalDateTime, 19) as Date) "
                 + ", Capture_Date "

                 + " FROM [RRDM_Reconciliation_ITMX].[dbo].BULK_" + InOriginFileName
                 // GET Only the valid ones
                 // + " WHERE (ResponseCode = '00' And Cast(TransactionAmount as decimal(18,2))> 0 ) "
                 + " WHERE (Cast(Transaction_Amount as decimal(18,2))<> 0 ) "
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                try
                {
                    ...
                    if (Environment.UserInteractive & TEST == true)
                    {
                        System.Windows.Forms.MessageBox.Show("Records Inserted For HOST_TO_HOST_124" + Environment.NewLine
                                 + "..:.." + stpLineCount.ToString());
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During HOST_TO_HOST_124 insert of records";
                    stpReturnCode = -1;
            ...
            stpErrorText += DateTime.Now + "_" + "Insert in HOST_TO_HOST_124 with records.." + stpLineCount.ToString() + "\r\n";
            stpErrorText += DateTime.Now + "_" + "Time Taken in Ms " + commandExecutionTimeInMs.ToString() + "\r\n";
            Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);
            ...
            //
            // UPDATE  with IST TWIN
            //

            SQLCmd =
          " UPDATE [RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_124] "
          + " SET "
          ...
          + ",AmtFileBToFileC = t2.SeqNo " // indication that this record is updated 
           ...
           + " FROM [RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_124] t1 "
          + " INNER JOIN [RRDM_Reconciliation_ITMX].[dbo].[Switch_IST_Txns_TWIN] t2"
          ...
          ; // For not processed yet records
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During HOST_TO_HOST_124 Updating For date and time";
                    CatchDetails(ex);
                    return;
                }

            stpErrorText += DateTime.Now + "_" + "Update Date and Time from IST.TWIN.." + stpLineCount.ToString() + "\r\n";
            stpErrorText += DateTime.Now + "_" + "Time Taken in Ms " + commandExecutionTimeInMs.ToString() + "\r\n";
            Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);
          ...
          + " WHERE  "
          + "(t1.Processed = 0  ) "
          + " AND (t2.Processed = 0 OR (t2.Processed=1 AND t2.Comment = 'Reversals') ) "
               + " AND (t2.MatchingCateg = '" + PRX + "286' ) "
          ...
          ; // For not processed yet records
            ...
            // INSERT In REVERSALS for HOST_TO_HOST_124
            string FileId = InOriginFileName;
          ...
          + ", a.TXNSRC As TXNSRC_4,b.TXNSRC as TXNSRC_2 "
          + ", a.TXNDEST As TXNDEST_4,b.TXNDEST as TXNDEST_2 "
          + " FROM [RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_124] A "
          + " LEFT JOIN [RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_124] B "
          + " ON A.TerminalId = B.TerminalId "
          //+ " AND A.CardNumber = B.CardNumber "
          + " AND A.TransAmt = B.TransAmt "
          + " AND A.RRNumber = B.RRNumber "
       ...
       ;
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    conn.Close();
                    ErrorFound = true;
                    stpErrorText = stpErrorText + "Cancel At _Reversals_HOST_TO_HOST_124";
                    stpReturnCode = -1;
            ...
            Counter = 0;
            //FileId 
            string TableId = "[RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_124]";
            int SeqNo_2;
            int SeqNo_4;
            ...
            stpErrorText += DateTime.Now + "_" + "HOST_TO_HOST_124 Finishes with SUCCESS.." + "\r\n";
        ...
        //*******************
        //// CREATE HOST_TO_HOST_EBC
        /////****************
        ///
                //*******************
        //// SAME AS MEEZA_GLOBAL LCL
        /////****************
        public void InsertRecords_GTL_HOST_TO_HOST_EBC(string InOriginFileName, string InFullPath
                                           , int InReconcCycleNo)
        {
            ...
            SQLCmd = " BULK INSERT [RRDM_Reconciliation_ITMX].[dbo].BULK_" + InOriginFileName
                          ...
                          + " ,ROWs_PER_BATCH=15000 "
                          // + ",FIELDTERMINATOR = '\t'"
                          + ",FIELDTERMINATOR = '" + InDelimiter + "'"  // Csv file 
                         + " ,ROWTERMINATOR = '\r\n' )"
                          ;
            ...
            SQLCmd = "INSERT INTO [RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_EBC]  "
                + "( "
                 + " [OriginFileName] "
                 ...
                 +", '124' " // TXNSRC
                                  + ", '123' " // TXNDEST
                //+ ",case " // Origin 
                //+ " WHEN TerminalOwner= 'ABEN' THEN '124'  " // ABE
                                         //+ " else '8' " // Other 
                                         //+ "end "

                 //+ ",case " // Destination
                 //+ " WHEN TerminalOwner= 'ABEN' THEN '123'  " // ABE
                 //+ " else '1' "
                 //+ "end "
                 + ", cast(LocalDateTime as Date) "
                 + ", SettlementDate "
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                try
                {
                    ...
                    if (Environment.UserInteractive & TEST == true)
                    {
                        System.Windows.Forms.MessageBox.Show("Records Inserted For HOST_TO_HOST_EBC" + Environment.NewLine
                                 + "..:.." + stpLineCount.ToString());
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    stpErrorText = stpErrorText + "Cancel During HOST_TO_HOST_EBC insert of records";
                    stpReturnCode = -1;
            ...
            stpErrorText += DateTime.Now + "_" + "Insert in HOST_TO_HOST_EBC with records.." + stpLineCount.ToString() + "\r\n";
            stpErrorText += DateTime.Now + "_" + "Time Taken in Ms " + commandExecutionTimeInMs.ToString() + "\r\n";
            Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);
            ...
            SQLCmd = " UPDATE [RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_EBC] "
                         + " SET "
                         + "   [SET_DATE] = DATEADD(day, -1 , SET_DATE )    "
                         + " WHERE LoadedAtRMCycle =  @LoadedAtRMCycle "
                         ;
          ...
          + ", a.TXNSRC As TXNSRC_4,b.TXNSRC as TXNSRC_2 "
          + ", a.TXNDEST As TXNDEST_4,b.TXNDEST as TXNDEST_2 "
          + " FROM [RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_EBC] A "
          + " LEFT JOIN [RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_EBC] B "
          + " ON A.TerminalId = B.TerminalId "
          //+ " AND A.CardNumber = B.CardNumber "
          + " AND A.TransAmt = B.TransAmt "
          + " AND A.RRNumber = B.RRNumber "
       ...
       ;
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                try
                {
                    ...
                    if (Environment.UserInteractive & TEST == true)
                    {
                        System.Windows.Forms.MessageBox.Show("REVERSAL ENTRIES Inserted for HOST_TO_HOST_EBC" + Environment.NewLine
                                  + "..:.." + Counter.ToString());
                    }
                ...
                catch (Exception ex)
                {
                    conn.Close();
                    ErrorFound = true;
                    stpErrorText = stpErrorText + "Cancel At _Reversals_HOST_TO_HOST_EBC";
                    stpReturnCode = -1;
            ...
            Counter = 0;
            //FileId = "Flexcube";
            string TableId = "[RRDM_Reconciliation_ITMX].[dbo].[HOST_TO_HOST_EBC]";
            int SeqNo_2;
            int SeqNo_4;
            ...
            stpErrorText += DateTime.Now + "_" + "HOST_TO_HOST_EBC Finishes with SUCCESS.." + "\r\n";
...
+ " ORDER BY ATMId, TransactionSequence, CurrencyCode ";
       ...
       ;
                                           ...
                                           , int InReconcCycleNo)
        {
            ...
            // DELETE DUBLICATES FROM VISA FILE
            ...
            SQLCmd = " INSERT INTO [RRDM_Reconciliation_ITMX].[dbo].BULK_VISA_CARD_ALL " //  KEEP ALL RECORDS IN THIS FILE
                + " Select * " + " ,@RMCycle " + " FROM [RRDM_Reconciliation_ITMX].[dbo].BULK_" + InOriginFileName;
            ...
            using (SqlConnection conn = new SqlConnection(recconConnString))
                ...
                catch (Exception ex)
                {
                    conn.StatisticsEnabled = false;
                    conn.Close();
                    stpErrorText = stpErrorText + "Cancel During Creation of all Cancel";
                    stpReturnCode = -1;
            ...
            stpErrorText += DateTime.Now + "_" + "Stage_03_Insert_To Visa Bulk Finishes..Records:.." + Counter.ToString() + "\r\n";
            stpErrorText += DateTime.Now + "_" + "Time Taken in Ms " + commandExecutionTimeInMs.ToString() + "\r\n";
            Flog.Update_stpErrorText(WFlogSeqNo, stpErrorText);
            ...
            SQLCmd =
                ...
                + "( "
                 ...
                 + ", Trim(Terminal) "
                  + " , ISNULL(CAST((Trandate + ' '+TIME) as datetime), '1900-01-01')"
                 + ",case "
                 + " when [DC] = 'DR' THEN 21  " // Opposite for Visa 
                 + " when [DC] = 'CR' THEN 11  "
                 + " else 0 "
                 + "end "
                 + ",'Visa txn at ABE ATM ' "//  in place of Transaction Description 
                  + ",case "
                 + " when [Cur] = 'EGP' THEN '818'  " // Opposite for Visa 
                 + " else Cur "
                 + " End "
                 + ", TRIM(replace(Tran_Amt, ',', '')) as Decimal "

                 + ",TRIM([retREfNo]) "
                 + ",CAST(TRIM([TRACE]) as Int) "

                  + ", ISNULL(left([CardNumber], 6) + '******' + right([CardNumber], 4), '') "  // CARD  
                                                                                                // + ",[CARD] "  // 
                     + ",case "
                     + " when TRIM([RS]) = '00' THEN '0'  "
                     + " else TRIM([RS]) "
                     + "end "
                 + ", @LoadedAtRMCycle"
                  + ", @Operator"
                 ...
                 + " , ISNULL(CAST(Trandate as date), '1900-01-01')" // Net Transactions Date

                 + " , ISNULL(CAST(Stldate as date), '1900-01-01')" // Setlement Date

                 + " FROM [RRDM_Reconciliation_ITMX].[dbo].BULK_" + InOriginFileName
                 + "   "
                + "  WHERE TRIM(Tran_Amt) <> '0.00' and TRIM(Stl_amt)  <> '0.00'  AND sp = '02' "
                + " ";
    ...
    + " AND t1.Minutes_Date = t2.Minutes_Date"
    //+ " AND t1.AccNumber = t2.AccNo "
    + " WHERE  (t1.IsMatchingDone = 0 AND LEFT(ltrim(rtrim(t1.CardNumber)),6) = '123456' AND t1.Origin='Our Atms' ) "
    ...
    + " WHERE  (t1.IsMatchingDone = 0  AND t1.Origin='Our Atms' ) "; // For not processed yet records
        ...
        public void DeleteRecordsToSetStartingPoint(string InOperator, DateTime InDate, DateTime InCut_Off_Date, int InRMCycleNo)
        {
            ...
            string Filter1 = "Operator = '" + InOperator + "' AND Enabled = 1  ";
            Rs.ReadReconcSourceFilesToFillDataTable(WOperator, Filter1, InRMCycleNo);
            ...
            try
            {
                ...
                while (I <= (Rs.SourceFilesDataTable.Rows.Count - 1))
                {
                    ...
                    int WSeqNo = (int)Rs.SourceFilesDataTable.Rows[I]["SeqNo"];
                    string WSourceFileId = (string)Rs.SourceFilesDataTable.Rows[I]["SourceFileId"];
                    ...
                    string SQLCmd2 = "DELETE FROM [RRDM_Reconciliation_ITMX].[dbo]." + WSourceFileId
                   + " WHERE TransDate  < @TransDate   "
                    ...
                    using (SqlConnection conn = new SqlConnection(recconConnString))
                        ...
                        catch (Exception ex)
                        {
                            ...
                            CatchDetails_2(ex, "Origin:878" + WSourceFileId);
                        }
            ...
            // Leave it here - it is not included in files 

            string SQLCmd = "DELETE FROM [RRDM_Reconciliation_ITMX].[dbo].[Switch_IST_Txns_TWIN] " // Leave it here 
             + " WHERE TransDate  < @TransDate   "
             + "   " // 
             ;
