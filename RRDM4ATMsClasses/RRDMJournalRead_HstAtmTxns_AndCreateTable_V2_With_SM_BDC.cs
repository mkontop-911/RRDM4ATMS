...
namespace RRDM4ATMs
{
    public class RRDMJournalRead_HstAtmTxns_AndCreateTable_V2_With_SM_BDC : Logger
    {
        public RRDMJournalRead_HstAtmTxns_AndCreateTable_V2_With_SM_BDC() : base() { }
       ...
       // int InternalCounter;
                                                             ...
                                                             int InAtmsReconcGroup, string InAtmNo, int InFuid, int InMode)
        {
            ...
            if (WMode == 3)
            {
                ...
                //Ua.ReadUserAccessToAtmsByAtm(WAtmNo); 
                //if (Ua.RecordFound & Ua.Reconciliation == true)
                //{
                //    // Transactions should coome sorted by date = will be loaded by another process
                //    return; 
                //}
            ...
            StartGroupDateTime = DateTime.Now;
                       ...
                       new SqlConnection(connectionString))
                        try
                        {
                            ...
                            using (SqlDataAdapter sqlAdapt = new SqlDataAdapter(SqlString, conn))
                            {
                                ...
                                Perform.InsertPerformanceTrace(WOperator, WOperator, 1, "Read HST", WAtmNo, BeforeCallDtTime, DateTime.Now, Message);
                                ...
                                while (K <= (HstAtmTxnsDataTable.Rows.Count - 1))
                                {
                                    ...
                                    if (TransactionType != 99 & Result.Substring(0, 2) == "OK" & TranAmount > 0)
                                    {
                                        // OK
                                    }
                                    else
                                    ...
                                    ResponseCode = (string)HstAtmTxnsDataTable.Rows[K]["ResponseCode"];

                                    if (ResponseCode == "000" || ResponseCode == "00") ResponseCode = "0";

                                    CardNo = (string)HstAtmTxnsDataTable.Rows[K]["cardnum"];

                                    if (TransactionType == 11 || TransactionType == 33) AccNo = (string)HstAtmTxnsDataTable.Rows[K]["acct1"];
                                    else
                                    {
                                        // This is deposit
                                    ...
                                    // TimeSpan Time2 = TRanDate.TimeOfDay;
                                    // 
                                   // if (TRanDate == OLD_TRanDate)
                                    //{

                                   //     InternalCounter = InternalCounter + 1;
                                        // Add Seconds
                                     //   TRanDate = TRanDate.AddSeconds(InternalCounter);
                                   // }
                                   // else
                                   // {
                                        // Keep Previous TRanDate
                                    //    InternalCounter = 0;
                                   //     OLD_TRanDate = TRanDate;
                                  //  }
                                    ...
                                    //if (TransactionType == 77)
                                    //{
                                    //    Method_HandlingSupervisorMode(WAtmNo, trandesc);
                                    //}
                                        ...
                                        & Result.Substring(0, 2) == "OK" & TranAmount > 0)
                                    {
                                        ...
                                        if (WOperator == "BCAIEGCX")
                                        {
                                            ...
                                            if (CardNo.Substring(0, 6) == "526402" || CardNo.Substring(0, 6) == "507803")
                                            {
                                                //if (CardNo.Substring(0, 6) == "507803")
                                                //{
                                                //    WMatchingCategoryId = "BDC201";
                                                //}
                                                WMatchingCategoryId = "BDC201"; // DEBIT CARDS
                                            }
                                            else
                                            {
                                                WMatchingCategoryId = "BDC299"; // ALL OTHERS 
                                            }
                                ...
                                // ****************INSERT BY PANICOS**************
                                Message = "PROCESS RECORDS AND CREATE DATA_TABLE FOR ATM..." + WAtmNo + " NUMBER OF RECs.." + K.ToString();
                                ...
                                Perform.InsertPerformanceTrace(WOperator, WOperator, 1, "UPDATE HST AS PROCESS", WAtmNo, BeforeCallDtTime, DateTime.Now, Message);
        ...
        private void Method_HandlingSupervisorMode(string WAtmNo, string In_SM_Mode)
        {
            ...
            if (W_SM_Mode == "SM-START" & TransactionType == 77) // SUPERVISOR MODE START = THIS IS THE END OF REPL CYCLE 
            {
                Message = "Start Replenishment For ATM : " + WAtmNo + " Done on : " + TRanDate.ToString();

                Perform.InsertPerformanceTrace(WOperator, WOperator, 2, "LoadTrans", WAtmNo, BeforeCallDtTime, DateTime.Now, Message);
                //if (ShowMessage == true)
                //{
            ...
            if (W_SM_Mode == "SM-END" & TransactionType == 77) // YESTERDAYS SM 
            {
