...
namespace RRDM4ATMs
{
    public class RRDMAtmsClass
    {
        ...
        public DataTable ATMsBasicSelected = new DataTable();
 ...
 // READ ATMs and created table as per request.
        ...
        public void ReadAtmAndFillTable(string InMode, string InSelectionCriteria, int InGroup ,string InAtmNo )
        {
            RecordFound = false;
            ErrorFound = false;
            ErrorOutput = "";
            ...
            ATMsBasicSelected = new DataTable();
            ATMsBasicSelected.Clear();

            TotalSelected = 0;

            // DATA TABLE ROWS DEFINITION 
            ATMsBasicSelected.Columns.Add("AtmNo", typeof(string));
            ATMsBasicSelected.Columns.Add("ReplCycle", typeof(int));
            ATMsBasicSelected.Columns.Add("AtmName", typeof(string));
            ATMsBasicSelected.Columns.Add("RespBranch", typeof(string));
                ...
                try
                {
                            ...
                            new SqlCommand(SqlString, conn))
                    {
                        ...
                        while (rdr.Read())
                        {
                            ...
                            if (ActiveAtm == true)
                            {
                                DataRow RowSelected = ATMsBasicSelected.NewRow();

                                RowSelected["AtmNo"] = AtmNo;
                                RowSelected["AtmName"] = AtmName;
                                RowSelected["RespBranch"] = Branch;
                               
                                // ADD ROW
                                ATMsBasicSelected.Rows.Add(RowSelected);
                            }
        ...
        public void DeleteNotActiveATMBasic(string InAtmNo)
        {
            ...
            using (SqlConnection conn =
                ...
                try
                {
                    conn.Open();
                    using (SqlCommand cmd =
                        new SqlCommand("DELETE FROM dbo.TableATMsBasic "
                            + " WHERE AtmNo = @AtmNo ", conn))
                    {
                        cmd.Parameters.AddWithValue("@AtmNo", InAtmNo);
                    ...
                    using (SqlCommand cmd =
                    new SqlCommand("DELETE FROM dbo.TableATMsCost "
                        + " WHERE AtmNo = @AtmNo ", conn))
                    {
                        cmd.Parameters.AddWithValue("@AtmNo", InAtmNo);
                ...
                catch (Exception ex)
                {
                    conn.Close();
                    ErrorFound = true;
                    ErrorOutput = "An error occured in DeleteNotActiveATMBasic(string InAtmNo) ............. " + ex.Message;
