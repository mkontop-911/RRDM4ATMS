...
namespace RRDM4ATMs
{
    public class RRDMRepl_SupervisorMode_Master_Recycle : Logger
    {
        public RRDMRepl_SupervisorMode_Master_Recycle() : base() { }
                                                             ...
                                                             int InMode, string InFrom)
        {
            ...
            WMode = InMode; // 3 Normal Process that takes the last in progress cycle and creates a new one
                            ...
                            // 4 is USED for correction coming from 153 
            ...
            try
            {
                ...
                if ((WMode == 3 || WMode == 2 || WMode == 4) & SM_Management == "YES")
                {
                    ...
                    SM.Update_SM_Record_NULL_Values_ONE(WLoadedAtRMCycle); // This is done for non processed and NULLS 
                    SM.Update_SM_Record_NULL_Values_TWO();
                    SM.Update_SM_Record_NULL_Values_THREE();

                    // DELETE ZERO CASES where two same SM
                    SM.Read_SM_AND_FillTable_For_Doubles(WLoadedAtRMCycle); 
                    ...
                    while (I <= (SM.DataTable_SM.Rows.Count - 1))
                    {
                        ...
                        {
                            // It is not valid 
                            // At the end we set it to Process mode = -6 
                        }

                        Am.ReadAtmsMainSpecific(SM.AtmNo);
                        ...
                        {
                            MessageBox.Show("Open Manually the ATMNo:.." + SM.AtmNo
                                + "This inconsistency was created from crazy Journals "
                                );
                            I = I + 1;
                            continue;
                        }

                        //if (SM.AtmNo == "00000520")
                        //{
                        //    SM.AtmNo = "00000520";
                        //}
                        

                        if (SM.AdditionalCash == "N") // THIS FULL REPL and Not Additional cash only 
                        {
                            // CHECK AND CORRECT LAST CLEAR DATE

                            Ta.FindNextAndLastReplCycleId(SM.AtmNo);

                            if (Ta.RecordFound == true)
                            {
                                if (SM.SM_LAST_CLEARED == SM.SM_dateTime_Start)
                                {
                                    // CASE where there is no Last Clear DATE
                                    SM.Update_SM_RecordLastClear(WSeqNo, Ta.SesDtTimeEnd.AddMinutes(2));
                                ...
                                // HERE WE READ THE Ta with process mode = -1

                                if ((SM.SM_LAST_CLEARED.Date < Ta.SesDtTimeStart.Date & SM.SM_dateTime_Start.Date > Ta.SesDtTimeStart.Date) & Correct_Process6 == false & SM.SM_LAST_CLEARED.Date != NullPastDate)
                                {
                                    //SM.SM_LAST_CLEARED.Date = Ta.SesDtTimeStart.Date; 

                                    SM.Update_SM_RecordLastClear(WSeqNo, Ta.SesDtTimeStart);
                            ...
                            else
                            {
                                ...
                                {
                                    // CASE where there is no Last Clear DATE
                                    SM.Update_SM_RecordLastClear(WSeqNo, SM.SM_LAST_CLEARED.AddMinutes(-9));
                            ...
                            if (Ta.RecordFound == true & SM.IsValid == true)
                            {
                                // MAKE ANOTHER CHECK if previous cycle it was valid
                                Ta.ReadSessionsStatusTraces(SM.AtmNo, Ta.PreSes);
                                if (Ta.ProcessMode != -6)
                                {
                                    // Additional Check if values read are correct
                                    // USE 12 Function 
                                    // Leave it 12 
                                    Na.ReadSessionsNotesAndValues(SM.AtmNo, Ta.SesNo, 12);

                                    if ((Na.Cassettes_1.InNotes != SM.ATM_total1
                                        || Na.Cassettes_2.InNotes != SM.ATM_total2
                                        || Na.Cassettes_3.InNotes != SM.ATM_total3
                                        || Na.Cassettes_4.InNotes != SM.ATM_total4)
                                        & Ta.SesDtTimeStart.Date == SM.SM_LAST_CLEARED.Date
                                        )
                                    {
                                        SM.IsValid = false;
                                    }
                                    ...
                                    {
                                        // Leave value as found in "SM.Read_SM_Record_Specific(WSeqNo)"
                                    }
                                }
                        ...
                        if (SM.AdditionalCash == "N") // THIS IS NOT FOR ADDING CASH ONLY
                        {
                            ...
                            if (IsRepeat == false) //
                            {
                                ...
                                TRanDate = TRanDate.AddMinutes(-2); // Subtract minutes to avoid cases 
                                                                    // Where next  txn has the same time
                                                                    // As the supervisor mode finish 

                                if (int.TryParse(SM.after_Repl_trace, out int result))
                                {
                                    TraceNo = result;
                                }
                                else
                            ...
                            SM.Update_SM_Deposit_analysis(WAtmNo, SavedFuid);
                            // Replenishment dates
        ...
        private void Method_SupervisorMode_Start(string InAtmNo)
        {
            ...
            if (IsRepeat == false)
            {
                ...
                else
                {
                    ...
                    {
                        Ta.SesDtTimeStart = SM.SM_LAST_CLEARED.AddMinutes(1);
                    }
                }
                ...
                {
                    Ta.SesDtTimeEnd = SM.SM_dateTime_Finish.AddMinutes(-1);
                }
        ...
        private void Method_SupervisorMode_ATM_FINISH_SM(string InAtmNo)
        {
            ...
            )
            {
                             ...
                             & Correct_Process6 == false & Correct_Process0 == false & IsCameLate == false)
                {
                    ...
                    Sa.CreateNewSession(WAtmNo, WSignedId, TRanDate.AddMinutes(3), OldTraceNo, WLoadedAtRMCycle); // The necessary records are created and the new Session No is available 
                    ...
                    Ta.SesDtTimeEnd = Ta.SesDtTimeStart.AddMinutes(3);
