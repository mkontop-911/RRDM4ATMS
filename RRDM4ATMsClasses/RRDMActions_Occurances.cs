...
namespace RRDM4ATMs
{
    public class RRDMActions_Occurances : Logger
    {
        public RRDMActions_Occurances() : base() { }
        ...
        public DateTime U_TransDate; // This is the date of the transaction not the date of the action
        ...
        private void ReadActionOccuranceFields(SqlDataReader rdr)
        {
            ...
            CR_AMOUNT = (decimal)rdr["CR_AMOUNT"];
            DR_AMOUNT = (decimal)rdr["DR_AMOUNT"];
            U_TransDate = (DateTime)rdr["U_TransDate"];
        ...
        //
        public void ReadActionsOccurancesAndFillTable_Small_2_OSMAN(string InSelectionCriteria)
        {
            RecordFound = false;
            ErrorFound = false;
            ErrorOutput = "";
            ...
            string DataBase = "ATMS_SAMIH_For_Osman";
        ...
        public void ReadActionsOccurancesBySelectionCriteriaToGetTotals(string InSelectionCriteria)
        {
            ...
            using (SqlConnection conn =
                ...
                try
                {
                    ...
                    if (Current_ExcessBalance == 0 & Current_ShortageBalance == 0 & Current_DisputeShortage ==0 )
                    {
                        // Do nothing 
                    }
                    else
                    {
                        ...
                        if ((Current_ExcessBalance == -Current_ShortageBalance) & (Current_ExcessBalance == -Current_DisputeShortage))
                        {
                        ...
                        {  
                            ...
                            if (-Current_DisputeShortage != 0)
                            {
                                Current_ExcessBalance = Current_ExcessBalance - (-Current_DisputeShortage);
                                Current_ShortageBalance = 0;
                            }

                        }
                        if (-Current_ShortageBalance > Current_ExcessBalance || -Current_DisputeShortage > Current_ExcessBalance)
                        {
                            ...
                            if (-Current_DisputeShortage > Current_ExcessBalance)
                            {
                                Current_DisputeShortage = (-Current_DisputeShortage) + Current_ExcessBalance;
                                Current_ExcessBalance = 0;
                            }
        ...
        public void ReadActionsOccurancesBySelectionCriteriaToGetTotals_Based_ON_dates
                                          (string InSelectionCriteria, DateTime DtTmStart, DateTime DtTmEnd)
        {
            RecordFound = false;
            ErrorFound = false;
            ErrorOutput = "";
            ...
            SqlString = " SELECT * "
                      + " FROM [ATMS].[dbo].[Actions_Occurances]"
                      + InSelectionCriteria
                      + " AND (U_TransDate BETWEEN @DateFrom AND @DateTo) "
                       + " Order By SeqNo  ";
            ...
            using (SqlConnection conn =
                ...
                try
                {
                    ...
                    using (SqlCommand cmd =
                        new SqlCommand(SqlString, conn))
                    {
                        cmd.Parameters.AddWithValue("@DateFrom", DtTmStart);
                        cmd.Parameters.AddWithValue("@DateTo", DtTmEnd);
        ...
        public void ReadActionsOccurancesBySelectionCriteriaToGetTotals_2_OSMAN(string InSelectionCriteria)
        {
            RecordFound = false;
            ErrorFound = false;
            ErrorOutput = "";
        ...
        public DataTable TxnsTableAllCycles_2;
        public DataTable TxnsTableAllCycles_Details_2;
        ...
        public decimal TotalExcess_ATM;
        public decimal TotalRefunded_ATM;
        public decimal TotalShortages_ATM;
        public decimal TotalRecovered_ATM;
            ...
            DateTime InToDt, int InMode)
        {
            ...
            if (InMode == 1)
            {
                if (InMode == 1) // Excess and Shortage 
                {
                    SqlString = " SELECT A.AtmNo "
                         + ", A.ReplCycle SesNo "
                        + ",B.SesDtTimeStart ,B.SesDtTimeEnd , B.AtmsReplGroup ,  A.CR_AMOUNT , A.DR_AMOUNT "
                     //+ " ,B.SesDtTimeStart ,B.SesDtTimeEnd , B.AtmsReplGroup "
                     + " FROM ATMS.[dbo].[Actions_Occurances]  A "
                     + " LEFT JOIN  ATMS.[dbo].SessionsStatusTraces B "
                     + " ON "
                     + " A.AtmNo = B.AtmNo "
                     + " AND A.ReplCycle = B.SesNo "
                     + "  WHERE (A.ShortAccID_1 = '40' OR A.ShortAccID_2 = '40') OR (A.ShortAccID_1 = '50' OR A.ShortAccID_2 = '50') and A.Stage = '03' "
                     + "  AND (CAST(B.SesDtTimeEnd As DATE) >= @FromDt  AND CAST(B.SesDtTimeEnd As DATE) <= @ToDt ) "
                     // + " Group by A.AtmNo"
                     + "   Group by A.AtmNo, A.ReplCycle ,B.SesDtTimeStart,B.SesDtTimeEnd, B.AtmsReplGroup ,  A.CR_AMOUNT , A.DR_AMOUNT "
                     // + ",B.SesDtTimeStart,B.SesDtTimeEnd, B.AtmsReplGroup "
                     + " ORDER by A.AtmNo , A.ReplCycle"
                     ;
                }
            }
        ...
        public void ReadRichPicture_ALL_ATMs_By_Selection_2(string InSignedId, string InOperator, string InSelectionCriteria, DateTime InFromDt,
                                                              DateTime InToDt, int InMode)
        {
            RecordFound = false;
            ErrorFound = false;
            ...
            string SQLCmd = " UPDATE " + DataBase + ".[dbo].[Actions_Occurances]"
                          + " SET [CR_AMOUNT] = DoubleEntryAmt "
                          + " WHERE(GL_Sign_1 = 'CR'  AND ShortAccID_1 in ('40', '50'))  "
                          + " OR(GL_Sign_2 = 'CR'  AND ShortAccID_2 in ('40', '50')) "
                          + " UPDATE " + DataBase + ".[dbo].[Actions_Occurances]"
                          + " SET [DR_AMOUNT] = DoubleEntryAmt "
                          + " WHERE(GL_Sign_1 = 'DR'  AND ShortAccID_1 in ('40', '50'))  "
                          + " OR(GL_Sign_2 = 'DR'  AND ShortAccID_2 in ('40', '50')) "
                          ;
            ...
            if (InMode == 12) // Excess 
            {
                SqlString = " SELECT A.AtmNo"
                    // +", A.ReplCycle "
                    + ", sum(A.CR_AMOUNT) as CR_AMOUNT, sum(A.DR_AMOUNT) as DR_AMOUNT ,count(*) As Cycles "
                 //   + " ,B.SesDtTimeStart ,B.SesDtTimeEnd , B.AtmsReplGroup "
                 + " FROM " + DataBase + ".[dbo].[Actions_Occurances]  A "
                 + " LEFT JOIN " + DataBase + ".[dbo].SessionsStatusTraces B "
                 + " ON "
                 + " A.AtmNo = B.AtmNo "
                 + " AND A.ReplCycle = B.SesNo "
                 + " WHERE (A.ShortAccID_1 = '40' OR A.ShortAccID_2 = '40') and A.Stage = '03' "
                 + "  AND (CAST(B.SesDtTimeEnd As DATE) >= @FromDt  AND CAST(B.SesDtTimeEnd As DATE) <= @ToDt ) "
                 + " Group by A.AtmNo"
                 //+ " Group by A.AtmNo, A.ReplCycle "
                 //+ ",B.SesDtTimeStart,B.SesDtTimeEnd, B.AtmsReplGroup "
                 + " ORDER by A.AtmNo "
                 ;
            }
            if (InMode == 14) // Shortage
            {
                SqlString = " SELECT A.AtmNo"
                    + ", sum(A.CR_AMOUNT) as CR_AMOUNT, sum(A.DR_AMOUNT) as DR_AMOUNT ,count(*) As Cycles "
                 //   + " ,B.SesDtTimeStart ,B.SesDtTimeEnd , B.AtmsReplGroup "
            ...
            if (InMode == 12) // Excess 
            {
                TxnsTableAllCycles_Details.Columns.Add("EXCESS", typeof(decimal));
                TxnsTableAllCycles_Details.Columns.Add("Refunded", typeof(decimal));
                TxnsTableAllCycles_Details.Columns.Add("Net Amount", typeof(decimal));
                TxnsTableAllCycles_Details.Columns.Add("Cycles Excess", typeof(int)); // 
            }
            if (InMode == 14) // Shortage 
            {
                TxnsTableAllCycles_Details.Columns.Add("SHORTAGE", typeof(decimal));
                TxnsTableAllCycles_Details.Columns.Add("Recovered", typeof(decimal));
                TxnsTableAllCycles_Details.Columns.Add("Net Amount", typeof(decimal));
                TxnsTableAllCycles_Details.Columns.Add("Cycles Short", typeof(int)); // 
            }

            TxnsTableAllCycles_Details.Columns.Add("Total Repl", typeof(string)); // Total Repl

            TxnsTableAllCycles_Details.Columns.Add("Perc", typeof(float)); // percentage 
            ...
            try
            {
                ...
                while (I <= K)
                {
                    ...
                    string WAtmNo = (string)TxnsTableAllCycles.Rows[I]["AtmNo"];
                    int WCycles = (int)TxnsTableAllCycles.Rows[I]["Cycles"];
                    // Read the rest 
                    if (InMode == 12) // Excess 
                    {
                        Excess = (decimal)TxnsTableAllCycles.Rows[I]["CR_AMOUNT"];
                        Refund = (decimal)TxnsTableAllCycles.Rows[I]["DR_AMOUNT"];
                    }
                    if (InMode == 14) // Shortage
                    {
                        Shortage = (decimal)TxnsTableAllCycles.Rows[I]["DR_AMOUNT"]; // DR FOR SHORTAGE
                        Recovered = (decimal)TxnsTableAllCycles.Rows[I]["CR_AMOUNT"];
                    ...
                    RowSelected["ATM_No"] = WAtmNo;
                    //RowSelected["Repl_No"] = WRepl_No;

                    if (InMode == 12) // Excess 
                    {
                        RowSelected["EXCESS"] = Excess;
                        RowSelected["Refunded"] = Refund;
                        RowSelected["Net Amount"] = Excess - Refund;
                        RowSelected["Cycles Excess"] = WCycles;

                    }
                    if (InMode == 14) // Shortage 
                    {
                        RowSelected["SHORTAGE"] = Shortage;
                        RowSelected["Recovered"] = Recovered;
                        RowSelected["Net Amount"] = Shortage - Recovered;
                        RowSelected["Cycles Short"] = WCycles;

                    }

                    int WReplCycles = ReadAndFindMaxTraceNo(DataBase, WAtmNo, InFromDt, InToDt);
                    RowSelected["Total Repl"] = WReplCycles;

                    float result = 0;
                    result = ((WCycles) * 100) / WReplCycles;

                    RowSelected["Perc"] = result;
                    
                    if (Environment.MachineName=="RRDM-PANICOS")
                    {
                        Ac.ReadAtm_OSMAN(WAtmNo);
                    }
                    else
                    {
                        Ac.ReadAtm(WAtmNo);
                    ...
                    if (InMode == 14) // Shortage 
                    {
                        TotalShortages = TotalShortages + Shortage;
        ...
        public void ReadRichPicture_ALL_ATMs_By_Selection_2_Actions(string InSignedId, string InOperator, string InSelectionCriteria, DateTime InFromDt,
                                                           DateTime InToDt, int InMode)
        {
            RecordFound = false;
            ErrorFound = false;
            ...
            if (InMode == 1) // Excess and Shortage 
            {
                SqlString = " SELECT A.AtmNo as ATM_No"
                     + ", A.ReplCycle as Repl_No "
                    + ",B.SesDtTimeStart ,B.SesDtTimeEnd , B.AtmsReplGroup ,  A.CR_AMOUNT , A.DR_AMOUNT "
                   //+ " ,B.SesDtTimeStart ,B.SesDtTimeEnd , B.AtmsReplGroup "
                 + " FROM " + DataBase + ".[dbo].[Actions_Occurances]  A "
                 + " LEFT JOIN " + DataBase + ".[dbo].SessionsStatusTraces B "
            ...
            using (SqlConnection conn =
                ...
                try
                {
                    ...
                    using (SqlDataAdapter sqlAdapt = new SqlDataAdapter(SqlString, conn))
                    {
                        ...
                        //Create a datatable that will be filled with the data retrieved from the command
                        sqlAdapt.SelectCommand.CommandTimeout = 300;
                        sqlAdapt.Fill(TxnsTableAllCycles_Details);
        ...
        public int ReadAndFindMaxTraceNo(string DataBase, string AtmNo, DateTime FromDt, DateTime ToDt)
        {
            RecordFound = false;
            ErrorFound = false;
            ErrorOutput = "";
            ...
            SqlString = " SELECT  A.AtmNo, sum(A.CR_AMOUNT) as CR_AMOUNT,count(*) As ALLReplCycles "
                 + " FROM " + DataBase + ".[dbo].[Actions_Occurances]  A "
                 + " LEFT JOIN " + DataBase + ".[dbo].SessionsStatusTraces B "
                 + " ON "
                 + " A.AtmNo = B.AtmNo "
                 + " AND A.ReplCycle = B.SesNo "
                 + " WHERE (A.ShortAccID_1 = '35') and A.Stage = '03' "
                 + "  AND (CAST(B.SesDtTimeEnd As DATE) >= @FromDt  AND CAST(B.SesDtTimeEnd As DATE) <= @ToDt"
                 + " AND A.AtmNo = @AtmNo ) "
                 + " Group by A.AtmNo "
                 + " ORDER by A.AtmNo "
                ...
                try
                {
                        ...
                        new SqlCommand(SqlString, conn))
                    {
                        ...
                        while (rdr.Read())
                        {
                            ...
                            ALLReplCycles = (int)rdr["ALLReplCycles"];
            ...
            return ALLReplCycles;
        }

        public void ReadRichPicture_ALL_ATMs_By_Selection_3_Per_ATM(string InSignedId, string InOperator, string InSelectionCriteria, DateTime InFromDt,
                                                            DateTime InToDt, int InMode)
        {
            RecordFound = false;
            ErrorFound = false;
            ...
            TotalExcess_ATM = 0;
            TotalShortages_ATM = 0;
            TotalRefunded_ATM = 0;
            TotalRecovered_ATM = 0;

            // UPDATE OCCUARANCE TABLE 
            ...
            TxnsTableAllCycles_2 = new DataTable();
            TxnsTableAllCycles_2.Clear();

            TxnsTableAllCycles_Details_2 = new DataTable();
            TxnsTableAllCycles_Details_2.Clear();

            if (InMode == 13) // Excess per ATM
            {
                SqlString = " SELECT A.AtmNo, A.ReplCycle "
                    + ", sum(A.CR_AMOUNT) as CR_AMOUNT, sum(A.DR_AMOUNT) as DR_AMOUNT "
                 //   + " ,B.SesDtTimeStart ,B.SesDtTimeEnd , B.AtmsReplGroup "
                 + " FROM " + DataBase + ".[dbo].[Actions_Occurances]  A "
                 + " LEFT JOIN " + DataBase + ".[dbo].SessionsStatusTraces B "
                 + " ON "
                 + " A.AtmNo = B.AtmNo "
                 + " AND A.ReplCycle = B.SesNo "
                 + InSelectionCriteria + " AND (A.ShortAccID_1 = '40' OR A.ShortAccID_2 = '40') and A.Stage = '03' "
                 + "  AND (CAST(B.SesDtTimeEnd As DATE) >= @FromDt  AND CAST(B.SesDtTimeEnd As DATE) <= @ToDt ) "
                 + " Group by A.AtmNo, A.ReplCycle "
                 //+ ",B.SesDtTimeStart,B.SesDtTimeEnd, B.AtmsReplGroup "
                 + " ORDER by A.Replcycle "
                 ;
            }
            if (InMode == 15) // Shortage per ATM 
            {
                SqlString = " SELECT A.AtmNo, A.ReplCycle "
                    + ", sum(A.CR_AMOUNT) as CR_AMOUNT, sum(A.DR_AMOUNT) as DR_AMOUNT "
                 //   + " ,B.SesDtTimeStart ,B.SesDtTimeEnd , B.AtmsReplGroup "
                 ...
                 + " A.AtmNo = B.AtmNo "
                 + " AND A.ReplCycle = B.SesNo "
                 + InSelectionCriteria + " AND  (A.ShortAccID_1 = '50' OR A.ShortAccID_2 = '50') and A.Stage = '03' "
                 + "  AND (CAST(B.SesDtTimeEnd As DATE) >= @FromDt  AND CAST(B.SesDtTimeEnd As DATE) <= @ToDt ) "
                 + " Group by A.AtmNo, A.ReplCycle "
                 //+ ",B.SesDtTimeStart,B.SesDtTimeEnd, B.AtmsReplGroup "
                 + " ORDER by A.Replcycle "
            ...
            using (SqlConnection conn =
                ...
                try
                {
                    ...
                    using (SqlDataAdapter sqlAdapt = new SqlDataAdapter(SqlString, conn))
                    {
                        ...
                        //Create a datatable that will be filled with the data retrieved from the command
                        sqlAdapt.SelectCommand.CommandTimeout = 300;
                        sqlAdapt.Fill(TxnsTableAllCycles_2);
            ...
            // DATA TABLE ROWS DEFINITION 
            TxnsTableAllCycles_Details_2.Columns.Add("ATM_No", typeof(string));
            TxnsTableAllCycles_Details_2.Columns.Add("Repl_No", typeof(int));

            TxnsTableAllCycles_Details_2.Columns.Add("Start_DATE", typeof(DateTime)); // Ses Start Date
            TxnsTableAllCycles_Details_2.Columns.Add("End_DATE", typeof(DateTime)); // SesDtTimeEnd
            if (InMode == 13) // Excess FOR ATM
            {
                TxnsTableAllCycles_Details_2.Columns.Add("EXCESS", typeof(decimal));
                TxnsTableAllCycles_Details_2.Columns.Add("Refunded", typeof(decimal));
                TxnsTableAllCycles_Details_2.Columns.Add("Net Amount", typeof(decimal));
            }
            if (InMode == 15) // Shortage For ATM
            {
                TxnsTableAllCycles_Details_2.Columns.Add("SHORTAGE", typeof(decimal));
                TxnsTableAllCycles_Details_2.Columns.Add("Recovered", typeof(decimal));
                TxnsTableAllCycles_Details_2.Columns.Add("Net Amount", typeof(decimal));
            }

            TxnsTableAllCycles_Details_2.Columns.Add("Settled", typeof(bool));

            TxnsTableAllCycles_Details_2.Columns.Add("Settled Date", typeof(string));

            TxnsTableAllCycles_Details_2.Columns.Add("Cit_Id", typeof(string)); // CIT
            TxnsTableAllCycles_Details_2.Columns.Add("Group", typeof(string)); // Group

            TxnsTableAllCycles_Details_2.Columns.Add("Rec_DATE", typeof(string)); // ReplFinDtTm

            TxnsTableAllCycles_Details_2.Columns.Add("Maker", typeof(string)); // Maker
            TxnsTableAllCycles_Details_2.Columns.Add("Auth", typeof(string)); // Authoriser
            ...
            try
            {
                ...
                int I = 0;
                int K = TxnsTableAllCycles_2.Rows.Count - 1;
                ...
                while (I <= K)
                {
                    ...
                    string WAtmNo = (string)TxnsTableAllCycles_2.Rows[I]["AtmNo"];
                    int WRepl_No = (int)TxnsTableAllCycles_2.Rows[I]["ReplCycle"];
                    if (InMode == 13) // Excess 
                    {
                        Excess = (decimal)TxnsTableAllCycles_2.Rows[I]["CR_AMOUNT"];
                        Refund = (decimal)TxnsTableAllCycles_2.Rows[I]["DR_AMOUNT"];
                    }
                    if (InMode == 15) // Shortage
                    {
                        Shortage = (decimal)TxnsTableAllCycles_2.Rows[I]["DR_AMOUNT"]; // DR FOR SHORTAGE
                        Recovered = (decimal)TxnsTableAllCycles_2.Rows[I]["CR_AMOUNT"];
                    }

                    // FILL TABLE

                    //NEW ROW
                    DataRow RowSelected = TxnsTableAllCycles_Details_2.NewRow();

                    RowSelected["ATM_No"] = WAtmNo;
                    RowSelected["Repl_No"] = WRepl_No;

                    Ta.ReadSessionsStatusTraces_2_OSMAN(WAtmNo, WRepl_No);

                    if (Ta.RecordFound == true)
                    {
                        RowSelected["Start_DATE"] = Ta.SesDtTimeStart;
                        RowSelected["End_DATE"] = Ta.SesDtTimeEnd;
                        RowSelected["Rec_DATE"] = Ta.Repl1.ReplFinDtTm.ToShortDateString();
                        RowSelected["Maker"] = Ta.Maker;
                        RowSelected["Auth"] = Ta.Authoriser;
                        RowSelected["Group"] = Ta.AtmsReplGroup;
                    }
                    ...
                    DateAfterThreeMonths = Ta.Repl1.ReplFinDtTm.Date.AddMonths(3);

                    if (DateAfterThreeMonths <= DateTime.Now.Date)
                    {
                        RowSelected["Settled"] = true;
                        RowSelected["Settled Date"] = DateAfterThreeMonths.ToShortDateString();
                    }
                    else
                    {
                        RowSelected["Settled"] = false;
                        RowSelected["Settled Date"] = "";
                    }

                    // Ac.ReadAtm(SavedAtmNo);
                    Ac.ReadAtm_OSMAN(WAtmNo);
                    ...
                    if (InMode == 13) // Excess 
                    {
                        ...
                        TotalExcess_ATM = TotalExcess_ATM + Excess;
                        TotalRefunded_ATM = TotalRefunded_ATM + Refund;
                    }
                    if (InMode == 15) // Shortage 
                    {
                        TotalShortages_ATM = TotalShortages_ATM + Shortage;
                        TotalRecovered_ATM = TotalRecovered_ATM + Recovered;
                    }

                    // ADD ROW
                    TxnsTableAllCycles_Details_2.Rows.Add(RowSelected);
        ...
        public void ReadActionsOccurancesAndFillTable_Small_Manual_AND_Other_Info(string InSignedId, int InRMCycle
                                                          , string InSelection, DateTime FromDate, DateTime ToDate, int InMode)
        {
            RecordFound = false;
            ErrorFound = false;
            ErrorOutput = "";
            ...
            // InMode = 35 Show Selected Actions
            ...
            if (InMode == 35)
            {
                SqlString = " SELECT * "
                     + " FROM [ATMS].[dbo].[Actions_Occurances]"
                     + " WHERE UniqueKeyOrigin = 'Master_Pool' and Stage = '03' "
                     + " AND ActionId in (" + InSelection + ")"
                     + " AND  CAST(ActionAtDateTime As DATE) >= @FromDate AND CAST(ActionAtDateTime As DATE) <= @ToDate"
                      + " Order By AtmNo , ActionAtDateTime  ";
            }
            ...
            if (InMode == 96)
            {
                SqlString = " SELECT * "
                     + " FROM [ATMS].[dbo].[Actions_Occurances]"
                     + " WHERE UniqueKeyOrigin = 'Master_Pool' AND RMCycle = @RMCycle and Stage = '03' AND OriginWorkFlow = 'Dispute' "
                      + " Order By AtmNo , ActionAtDateTime  ";
            }
            ...
            using (SqlConnection conn =
                        ...
                        new SqlCommand(SqlString, conn))
                    {
                        ...
                        while (rdr.Read())
                        {
                            ...
                            if (UniqueKeyOrigin == "Master_Pool")
                            {
                                ...
                                bool SOLO;
                                if (Mpa.SeqNo05 != 0 & Mpa.SeqNo05 != 0)
                                {
                                    SOLO = true;
                                }
                                else
                ...
                catch (Exception ex)
                {
                    ...
                    // MessageBox.Show("Check your input data.." + InSelection); 
        ...
        public void CreateActionsTxnsPerActionId(string InOperator, string InSignedId,
                             string InActionId, string InUniqueKeyOrigin, int InUniqueKey,
                              ...
                              string InMaker_ReasonOfAction, string InOriginWorkFlow
                              )
        {
            RecordFound = false;
            ErrorFound = false;
            ...
            // Read Occurances 
            // CHECK IF ALREADY EXIST TO CLEAR IT
            IsSuccess = true;
            ReadActionsOccurancesByUniqueKey(InUniqueKeyOrigin, InUniqueKey, InActionId);

            if (RecordFound == true)
            {
                if (Stage == "03" & Is_GL_Action == true)
                {
                    MessageBox.Show("Action Already Taken and authorised For this unique id.."+ InUniqueKey.ToString());
                    IsSuccess = false; 
                    return; 
                }
                else
       ...
       // DateTime U_TransDate;
                                 ...
                                 int InOccurance, string InMaker_ReasonOfAction, string InOriginWorkFlow)
        {
            ...
            if (InUniqueKeyOrigin == "Master_Pool_MOBILE")
            {
                ...
                string SelectionCriteria = " WHERE  SeqNo =" + InUniqueKey;

                string MasterTableName = "[ETISALAT].[dbo].[ETISALAT_TPF_TXNS_MASTER]";

                Mmob.ReadTransSpecificFromSpecificTable_MOBILE(SelectionCriteria, MasterTableName, 1, "ETISALAT");
                ...
                WAccNo = "Unknown Account No";
                U_TransDate = Mmob.TransDate;
            ...
            if (Ag.Is_GL_Action == true)
            {
                ...
                if (Ag.ShortAccID_1 == "20") // Customer Account
                {
                    if (IsT24_Version == true)
                    {
                        // NO Branch for Account Number 
                        FirstBranch = "N/A";
                    }
                    else
                    {
                ...
                {
                    SecondAccName = "Customer Account";
                }
                ...
                else
                {
                    ...
                    if (Acc.RecordFound == true)
                    {
                        ...
                        if (WCitId != "1000")
                        {
                            ...
                            {
                                SecondBranch = Acc.BranchId; // ID for the branch . If CIT Account doent belong to branch we write "N/A"
                            }
            ...
            using (SqlConnection conn =
                ...
                try
                {
                       ...
                       new SqlCommand(cmdinsert, conn))
                    {
                        ...
                        string dateString = U_TransDate.ToShortDateString(); // Invalid date (February 30 does not exist)
